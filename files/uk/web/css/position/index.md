...
- `static` (статичне)
  - : Елемент позиціонується згідно зі звичайним плином документа. Властивості {{cssxref("top")}}, {{cssxref("right")}}, {{cssxref("bottom")}}, {{cssxref("left")}} та {{cssxref("z-index")}} не мають _жодної дії_. Таке значення є усталеним.
- `relative` (відносне)

  - : Елемент позиціонується згідно зі звичайним плином документа, а тоді відступає вбік _відносно початкового положення_ на основі значень `top`, `right`, `bottom` та `left`. Відступ не вражає позицій жодних інших елементів; таким чином, простір, виділений для елемента у макеті сторінки, такий самий, як був би, якби його `position` мала б значення `static`.
...
- `sticky` (липкий)
  ...
  - : Елемент позиціонується згідно зі звичайним плином документа, а тоді відступає вбік відносно до свого _найближчого предка з прокручуванням_ та [контейнерного блока](/uk/docs/Web/CSS/Containing_block) (найближчого предка блокового рівня), включно з елементами таблиць, на основі значень властивостей `top`, `right`, `bottom` та `left`. Відступ не вражає позицій жодних інших елементів.

    Це значення завжди створює новий [контекст нагромадження](/uk/docs/Web/CSS/CSS_Positioning/Understanding_z_index/The_stacking_context). Зверніть увагу, що липкий елемент "прилипає" до свого найближчого предка, що має "механізм прокручування" (утворений встановленням властивості `overflow` у значення `hidden`, `scroll`, `auto` чи `overlay`), навіть коли такий предок не є найближчим справді прокручуваним предком. Це дієво перешкоджає будь-якій "липкій" поведінці (дивіться [GitHub-проблему W3C CSSWG](https://github.com/w3c/csswg-drafts/issues/865)).
...
Відносно позиціоновані елементи мають заданий відступ від звичайної позиції в документі, але, втім, їх відступ не вражає інші елементи. У прикладі нижче зверніть увагу на те, як інші елементи розміщені, неначе "Два" займає місце у своєму звичайному положенні.
...
<p>
  ...
  рядку є місце. Переповнюючі рядкові елементи
...
Обов‘язково вказати поріг в одній з властивостей: `top`, `right`, `bottom` чи `left`, щоб липке позиціонування поводилось як очікується.
