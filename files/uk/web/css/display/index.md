...
- {{CSSxRef("&lt;display-outside&gt;")}}
  ...
  - : Ці ключові слова вказують зовнішній тип представлення елемента, що по суті є його роллю у потоковому компонуванні:
    ...
    - `block`
      - : Елемент створює блокову коробку елемента, утворюючи розриви до та після елемента у звичайному потоці.
    - `inline`
      - : Елемент утворює одну або більше рядкових коробок елемента, що не утворюють розривів рядка до або після них. У нормальному потоці наступний елемент буде розташований на тому ж самому рядку, що й попередній, якщо в рядку достатньо місця для нього.

> **Зверніть увагу:** Браузери, що підтримують синтаксис із двома значеннями, отримуючи виключно зовнішнє значення, наприклад, `display: block`, чи `display: inline`, встановлюють внутрішнє значення у `flow`.
> Така логіка призводить до очікуваної поведінки; наприклад, описавши елемент як `block`, очікуємо, що нащадки такого елемента братимуть участь у блоковому та рядковому звичайному потоковому верстанні.
...
- {{CSSxRef("&lt;display-inside&gt;")}}
  ...
  - : Ці ключові слова вказують внутрішній тип представлення елемента, що визначає тип форматування контексту, в якому його вміст розташовано (припускаючи, що елемент не заміщується):
    ...
    - `flow` {{Experimental_Inline}}
      ...
      - : Елемент розкладає свій вміст за допомогою потокового верстання (блокового-та-рядкового верстання).
        Якщо його зовнішній тип представлення – `inline` чи `run-in`, і він бере участь у блоковому чи рядковому контексті форматування, то генерує рядкову коробку. Інакше – утворює блокову контейнерну коробку.
    ...
    - `flow-root`
      - : Елемент породжує блокову коробку елемента, що утворює новий [блоковий контекст форматування](/uk/docs/Web/Guide/CSS/Block_formatting_context), визначаючи розташування кореня форматування.
    - `table`
      - : Такі елементи поводяться неначе HTML-елементи {{HTMLElement("table")}}. Вони визначають коробку блокового рівня.
    - `flex`
...
> Така логіка призводить до очікуваної поведінки; наприклад, описавши елемент як `grid`, очікуємо, що коробка, створена для сіткового контейнера, буде коробкою блокового рівня.
...
Єдине значення `list-item` змусить елемент поводитись неначе пункт списку. Таке значення може використовуватись вкупі із властивостями CSS {{CSSxRef("list-style-type")}} та {{CSSxRef("list-style-position")}}.
...
> Якщо не вказане зовнішнє значення, головна коробка матиме тип зовнішнього представлення `block`.
...
- {{CSSxRef("&lt;display-internal&gt;")}}
  ...
  - : Деякі моделі компонування, наприклад, `table` та `ruby`, мають складну внутрішню структуру, з багатьма різними ролями, що можуть бути заповнені їх нащадками.
    ...
    - `ruby-base-container` {{Experimental_Inline}}
      - : Такі елементи поводяться неначе елементи HTML {{HTMLElement("rbc")}}, утворені як анонімні коробки.
...
- {{CSSxRef("&lt;display-box&gt;")}}
  ...
  - : Ці значення визначають, чи задає елемент візуальні рамки взагалі.
    ...
    - `contents`
      - : Такі елементи не породжують певної коробки самі по собі. Вони замінюються власними псевдокоробками та коробками своїх нащадків. Будь ласка, зверніть увагу, що специфікація "Представлення CSS, рівень 3" визначає, як значення `contents` мусить впливати на "незвичні елементи" – елементи, що не відображаються суто згідно з концепціями коробки CSS, наприклад, замінені елементи. Дивіться [Додаток B: Дія display: contents на незвичні елементи](https://drafts.csswg.org/css-display/#unbox)
        _У зв‘язку із вадою у браузерах, це значення наразі прибирає елемент із дерева доступності – читачі екрану не дивитимуться на те, що всередині. Дивіться розділ [Занепокоєння щодо доступності](#accessibility_concerns) нижче для заглиблення в цю проблему._
    - `none`
      - : Вимикає відображення елемента так, що він не впливає на верстання (документ візуалізується так, ніби елемент не існує). Усі дочірні елементи також не відображаються.
...
- {{CSSxRef("&lt;display-legacy&gt;")}}
  ...
  - : CSS 2 використовував однослівний синтаксис властивості `display`, вимагаючи окремих ключових слів для варіантів одного й того ж режиму компонування на рівні блока та рівні вбудування.
    ...
    - `inline-block`
      ...
      - : Елемент утворює коробку блокового елемента, що буде розташована в єдиному потоці з сусіднім вмістом так, ніби вона є єдиною рядковою коробкою (поводячись так, як поводився б замінений елемент).

        Це еквівалентно до `inline flow-root`.

    - `inline-table`
      - : Значення `inline-table` не має прямого відповідника у HTML. Такий елемент поводиться неначе HTML-елемент {{HTMLElement("table")}}, але як рядкова коробка, а не як коробка блокового рівня. Всередині табличної коробки діє контекст блокового рівня.
