...
Аби відцентрувати елемент, слід використати властивість `align-items`, щоб шикувати елемент за поперечною віссю, котра у цьому випадку є вертикальною віссю блока. Слід використати `justify-content`, аби шикувати елемент за основною віссю, котра у цьому випадку є горизонтальною рядковою віссю.
...
У цих настановах розглядаємо наступні властивості:
...
- {{cssxref("gap")}}, {{cssxref("column-gap")}} та {{cssxref("row-gap")}} — використовуються для створення вертикальних і горизонтальних розривів між flex-елементами.

Іще одна тема для розгляду – те, як автоматичні зовнішні поля можуть бути використані для шикування у flex-контейнерах.

## Поперечна вісь

Властивості `align-items` та `align-self` керують шикуванням елементів flex-розкладки на поперечній осі – у колонках, якщо значення властивості `flex-direction` – `row`, і у рядах, якщо `flex-direction` – `column`.

Погляньмо на використання шикування на поперечній осі у найпростішому прикладі flex-розкладки. Якщо додати контейнерові `display: flex`, то дочірні елементи стануть flex-елементами, впорядкованими у ряд. Вони будуть розтягнені, аби бути настільки ж високими, як найвищий елемент, оскільки такий елемент визначає висоту елементів на поперечній осі. Якщо flex-контейнер має явно вказану висоту, то всі елементи розтягнуться до його висоти, незалежно від того, скільки вмісту вони мають.
...
Досі розглядалась поведінка шикування у flex-контейнері за умови, що значення `flex-direction` – `row` (ряд), і мова використовує написання рядків згори донизу. Це означає, що головна вісь напрямлена вздовж ряду – горизонтально, а шикування поперечної осі рухає елементи вгору-вниз.
...
Досі у прикладах цієї статті шикувались елементи, або окремий елемент, всередині області, визначеної flex-контейнером. Якщо є flex-контейнер з кількома рядами вмісту, можна також використати властивість `align-content`, щоб керувати розподілом простору між рядами. У специфікації це описано як [нагромадження flex-рядків](https://drafts.csswg.org/css-flexbox/#align-content-property).

Щоб `align-content` працювала, flex-контейнер повинен мати більшу висоту, ніж необхідно для показу елементів. Тоді це працює на усіх елементах як на групі, і диктує поведінку вільного простору, а також шикування усієї групи елементів в ньому.
...
> **Зверніть увагу:** Значення `space-evenly` не описане специфікацією Flexbox і є пізнішим додатком до специфікації Box Alignment. Підтримка браузерами цього значення не є такою доброю, як для інших значень, описаних у специфікації Flexbox.
...
Якщо головна вісь збігається із блоковим напрямком (значення `flex-direction` – `column`), то `justify-content` розподілятиме простір між елементами у відповідному вимірі доти, доки у flex-контейнері більше не залишиться простору для розподілу.
...
Якщо змінити `flex-direction` на одне зі зворотних значень, елементи розставлятимуться починаючи від кінця осі у зворотному порядку до написання слів мовою документа. `flex-start` тоді означатиме кінець тієї осі – тож місце, де б відбувався перенос (при роботі з рядами), або кінець останнього параграфа тексту (при роботі з колонками).
