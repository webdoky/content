...
Комбінатор вкладеності `&` не додає ваги специфічності, а вкладені правила – додають. Що до специфічності та функціональності, то вкладеність вельми подібна до псевдокласу {{CSSxRef(":is", ":is()")}}.
...
Про [винятки `:not()`, `:is()`, `:has()` і вкладеності CSS](#vyniatky-not-is-has-i-vkladenist-css) – нижче.
...
При утворенні складних списків селекторів з [вкладеним CSS](/uk/docs/Web/CSS/CSS_Nesting) це поводиться точно так само, як псевдоклас `:is()`.
...
У блокові коду вище складний селектор `p, #fakeId` має специфічність, взяту із `#fakeId`, а також `span`, тож це дає специфічність `1-0-1` для `p span` і `#fakeId span`. Це специфічність, рівносильна специфічності селектора `:is(p, #fakeId) span`.
