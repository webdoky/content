...
/* Вираз з колірними каналами у відносних кольорах */
...
Є кілька речей, які варто мати на увазі при роботі з `calc()`:
...
- Математичні вирази, що залучають відсоткові значення для обчислення ширин і висот для табличних колонок, груп табличних колонок, табличних рядів, груп табличних рядів і табличних комірок, як в таблицях з автоматичним компонуванням, так і в таблицях з фіксованим, _можуть_ розглядатися так, як якби було задано `auto`.

- Функція `calc()` не може безпосередньо замінити числове значення у відсотковому типі; наприклад, `calc(100 / 4)%` – недійсний запис, а `calc(100% / 4)` – дійсний.

- Коли `calc()` використовується там, де очікується {{cssxref("&lt;integer&gt;")}}, то значення округлюється до найближчого цілого. Таким чином, `calc(1.4)` дасть значення `1`. Якщо дробова частина значення рівна `0.5`, то значення округлюється до більшого. Наприклад `calc(1.5)` дасть значення `2`, а `calc(-1.5)` округлиться до `-1`.

### Правила та найкращі підходи при роботі з `calc()`
...
### Підтримка обчислення колірних каналів у відносних кольрах
...
Коли всі змінні розгорнуті, значення `widthC` стає `calc(calc(100px / 2) / 2)`. Коли воно присвоєно властивості ширини за селектором `.foo`, то всі внутрішні функції `calc()` (незалежно від глибини своєї вкладеності) сплощуються до простих дужок. Таким чином, значенням властивості `width` зрештою буде `calc((100px / 2) / 2)`, що дорівнює `25px`. Стисло висловлюючись, `calc()` всередині `calc()` рівносильно використанню дужок.
...
В наступних абзацах вживається `calc()` вкупі з функціями [`rgb()`](/uk/docs/Web/CSS/color_value/rgb) і [`hsl()`](/uk/docs/Web/CSS/color_value/hsl), щоб підлаштувати значення кожного з колірних каналів на основі вихідного іменованого кольору.
