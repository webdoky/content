...
> **Зверніть увагу:** Приклади на цій сторінці використовують медіазапит CSS `@media` для ілюстративних потреб, однак базовий синтаксис залишається однаковим для всіх типів медіазапитів.
...
> **Зверніть увагу:** Файл стилів із медіазапитом, прикріпленим до його тега {{HTMLElement("link")}}, [будуть стягнені](https://scottjehl.github.io/CSS-Download-Tests/) навіть якщо запит обчислюється у `false`; стягнення відбудеться, але пріоритет такого стягнення буде куди нижчим.
> Попри це, такі стилі не будуть застосовані, якщо не і поки медіазапит не матиме значення `true`.
...
Попри те, що вебсайти зазвичай розробляють для відображення на екранах, можна захотіти створити стилі, що націлені на особливі пристрої, наприклад, принтери або звукові читачі з екрана.
...
Наприклад, наступна директива `@media` використовує два медіазапити, щоб націлитись як на пристрої з екраном, так на принтери:
...
Дивіться [тип медіа](/uk/docs/Web/CSS/@media#media_types) для отримання списку типів медіа.
...
Наприклад, можна застосувати певні стилі до широкоекранних моніторів, комп‘ютерів із мишами, або до пристроїв, що використовуються при низькому освітленні.
...
Оператор `and` також може поєднувати кілька ознак медіа в один медіазапит. Тим часом оператор `not` заперечує медіазапит, по суті обертаючи його звичайне значення до протилежного.
Оператор `only` не дає старим браузерам застосувати стилі.

> **Зверніть увагу:** У більшості випадків використовується тип медіа `all` – коли не вказаний інший.
...
> **Зверніть увагу:** Специфікація "Медіазапити, рівень 4" має прийнятну підтримку у сучасних браузерах, втім, деякі ознаки медіа підтримуються недостатньо добре.
> Дивіться [таблицю сумісності з браузерами `@media`](/uk/docs/Web/CSS/@media#Browser_compatibility) для заглиблення.
