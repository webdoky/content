...
Властивість [CSS](/en-US/docs/Web/CSS) **`cursor`** вказує курсор миші, котрий повинен бути показаний, коли вказівник миші розташований над елементом.

Встановлення курсора повинне повідомити користувачів про дії миші, котрі можуть бути виконані у поточному положенні, включаючи: вибір тексту, активацію помічника або контекстних меню, копіювання вмісту, зміну розміру таблиць, і так далі.
...
Властивість `cursor` встановлюється нулем або більшою кількістю значень `<url>`, розділених комами, з обов‘язковим ключовим словом у кінці.
...
- `<url>` {{optional_inline}}
  - : `url(…)` або розділений комами список `url(…), url(…), …` – посилання на файл зображення.
    Більш ніж одне значення {{cssxref("url()")}} може бути вказане як запасні варіанти, на випадок того, що якісь типи зображень курсорів не підтримуються.
    ...
    <table class="standard-table">
      ...
      <tbody>
        <tr style="cursor: auto">
          ...
          <td></td>
          <td>
            Користувацький фактор визначить курсор на основі поточного контексту. Наприклад, еквівалентно до <code>text</code>, коли вказівник знаходиться над текстом.
          </td>
        </tr>
...
Хоч специфікація не обмежує розмір зображення курсора, {{Glossary("user agent", "користувацькі фактори")}} зазвичай обмежують його, аби уникнути можливого зловживання.
Наприклад, на Firefox та Chromium зображення курсора усталено обмежені розміром 32x32 пікселів, і заміна курсора з використанням більших зображень буде в загальному просто проігнорована.

Перевірте таблицю "{{anch("Сумісність із браузерами")}}" щодо зауважень щодо обмежень розмірів курсора.
