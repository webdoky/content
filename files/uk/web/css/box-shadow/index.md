...
- `<offset-x>` `<offset-y>`
  - : Це два значення {{cssxref("&lt;length&gt;")}} для опису зсуву тіні. `<offset-x>` вказує зсув по горизонталі. Від‘ємні значення поміщають тінь зліва від елемента. `<offset-y>` вказує вертикальний зсув. Від‘ємні значення поміщають тінь згори від елемента. Дивіться {{cssxref("&lt;length&gt;")}} для інформації про можливі одиниці вимірювання.
...
Кожна тінь у списку (розглядаючи `none` як список нульової довжини) інтерполюється через кольорову (як колір) компоненту, а також компоненти x, y, розмиття та (коли припустимо) розмах (як довжина). Для кожної початкової тіні виконується перевірка параметру `inset`. Якщо обидві тіні є `inset` (або обидві не є `inset`), то значення інтерпольованої тіні збігатиметься з початковими стосовно цього параметру. Якщо ж у будь-якій парі вхідних тіней одна містить `inset`, а інша – ні, то весь набір тіней не інтерполюється. Якщо списки тіней мають різну довжину, то коротший список в кінці заповнюється тінями, чий колір – `transparent`, усі довжини – `0`, і для котрих `inset` вказано чи не вказано – згідно з вихідним списком, поки короткий список не відповідатиме за довжиною довгому спискові.
