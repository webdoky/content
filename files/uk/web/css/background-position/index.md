...
- `<position>`
  ...
  - : {{cssxref("&lt;position&gt;")}} – положення. Визначає координати X та Y, аби розташувати елемент відносно країв рамок елемента. Може визначатися за допомогою від одного до чотирьох значень. Якщо задано два значення, що не є ключовими словами, то перше з них представляє положення по горизонталі, а друге – по вертикалі. Коли задано лише одне значення, то друге значення вважається за `center`. Коли використовуються три або чотири значення, то значення довжини або відсотків є відступами для попередніх значень – ключових слів.
    ...
    **Синтаксис 2 значень:** одне значення задає X, а друге – Y. Кожне з них може бути:
    ...
    - Зверніть увагу, що: Якщо одне зі значень – `top` або `bottom`, то інше значення не може бути `top` або `bottom`. Якщо одне зі значень – `left` або `right`, то інше значення не може бути `left` або `right`. Це означає, що, наприклад, `top top` і `left right` – недійсні значення.
...
Слід згадати, що коли background-size має значення, рівне розмірові контейнера за відповідною віссю, то _відсоткове_ положення за цією віссю не матиме дії, тому що "різниця контейнер-зображення" буде нулем. Доведеться робити відступи за допомогою абсолютних значень.
...
/* Декілька зображень тла: Кожне зображення відповідає
   відповідній позиції, від першої до останньої. */
.examplethree {
