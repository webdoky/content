...
Під час стилізації колонки таблиці не можна задати стилі елементу {{HTMLElement("col")}}, адже комірки таблиці не є його дочірніми (як це можна робити для елемента ряду, {{HTMLElement("tr")}}). Псевдокласи, як-от `:nth-child()`, зручні для вибору всіх комірок однієї колонки.
