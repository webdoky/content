...
У цьому прикладі – таблиця з одним рядом, що містить шість комірок. Ряд задає `vertical-align` усталене значення `bottom`.

- Кожна з перших чотирьох комірок має власне значення `vertical-align`, і ці значення перемагають значення від ряду.
- П'ята комірка не задає жодного значення `vertical-align`, тому успадковує значення від ряду.
