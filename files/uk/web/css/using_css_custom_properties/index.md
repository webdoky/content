...
Почнімо з такого простого CSS, що застосовує один колір до елементів з різними класами:
...
```css
.two {
  color: var(--my-var, red); /* Червоний, якщо --my-var не визначена */
}
...
## Дійсність та значення

Класична концепція дійсності CSS, пов‘язана з усіма властивостями, не дуже корисна щодо користувацьких властивостей. Коли значення користувацьких властивостей розбираються, браузер не знає, де вони будуть застосовані, тож мусить, таким чином, вважати мало не всі значення _дійсними_.

На жаль, такі дійсні значення можуть бути використані, за допомогою синтаксису функції `var()`, у контексті, де вони можуть не мати змісту. Властивості та користувацькі змінні можуть породити недійсні викази CSS, що веде до нової концепції _дійсності при обчисленні._

## Що відбувається із недійсними змінними

Коли браузер зустрічає недійсну заміну `var()`, використовується початкове або успадковане значення властивості.

Розгляньмо фрагмент коду нижче.
...
1.  Перевірити, чи є колір властивості успадковуваним. Так, втім, `<p>` не має жодного предка із властивістю `color`. Тому – перехід до наступного кроку.
...
Колір параграфа не буде блакитним, тому що недійсне підставлення замінюється початковим значенням, а не запасним варіантом. Якби написати `color: 16px` без жодних замін змінних, це було б синтаксичною помилкою. Тоді використовувалось би попереднє оголошення.

> **Зверніть увагу:** Синтаксична помилка у парі властивості та значення CSS веде до ігнорування рядка із використанням каскадного значення, а недійсне підставлення – використання значення користувацької властивості, що є недійсним – не ігнорується, а веде до успадкування значення.

## Значення у JavaScript

Використання значень користувацьких властивостей у JavaScript аналогічне до використання стандартних властивостей..
...
> **Зверніть увагу:** Префікс користувацьких властивостей у ранній специфікації був `var-`, а пізніше був змінений на `--`. Firefox версії 31 і новіше відповідає новій специфікації. ({{bug(985838)}})

## Дивіться також

- {{cssxref("--*", "Користувацькі властивості")}}
