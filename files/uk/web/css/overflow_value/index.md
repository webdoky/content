...
> [!NOTE]
> Значення – ключове слово `overlay` є історичним псевдонімом значення `auto`. При застосуванні `overlay` смуги прокручення наносяться поверх вмісту, а не займають окреме місце.
