...
[Сіткове компонування CSS](/uk/docs/Web/CSS/CSS_Grid_Layout) запроваджує в CSS двовимірну сіткову систему. Сітки можуть використовуватися для компонування головних частин сторінки чи невеликих елементів користувацького інтерфейсу. Ця стаття знайомить з Сітковим компонуванням CSS та новою термінологією, що є частиною специфікації Сіткового компонування CSS рівня 1. Можливості, показані в цьому огляді, надалі пояснюються докладніше в решті цього посібника.
...
Відтепер всі безпосередні нащадки стали сітковими елементами. У веббраузері не буде помітна жодна різниця щодо того, як ці елементи виводилися до перетворення їх на сітку, адже сітка створила для елементів одноколонкову сітку. На цьому етапі може мати зміст користування [Сіткового дослідника (англ.)](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/how_to/examine_grid_layouts/index.html), доступного як частина Інструментів розробника Firefox. Якщо переглянути цей приклад у Firefox і дослідити сітку, то буде помітна невелика піктограма біля значення `grid`. Клацніть на неї – і сітка елемента з'явиться у вікні браузера.

![Використання Сіткового значника в DevTools для перегляду сітки](1-grid-inspector.png)
...
У цьому прикінцевому прикладі змішуються доріжки з абсолютним розміром та одиниці `fr`. Перша доріжка - 500 пікселів, тож від доступного простору віднімається фіксована ширина. Решта простору ділиться натроє й пропорційно присвоюється двом гнучким доріжкам.
...
Великі сітки з багатьма доріжками можуть використовувати запис `repeat()`, котрий дає змогу повторювати всі чи розділ переліку доріжок. Наприклад, таке визначення сітки:
...
Запис повторення може застосовуватися як частина переліку доріжок. В наступному прикладі – сітка з 20-піксельною доріжкою на початку, після якої – повторюваний розділ з 6 доріжок `1fr`, а потім – кінцева 20-піксельна доріжка.
...
Якщо щось розташовується поза визначеною сіткою, або якщо у зв'язку з кількість вмісту необхідно більше доріжок, то сітка створює ряди й колонки неявної сітки. Такі доріжки усталено автоматично калібруються, завдяки чому їх розмір заснований на вмісті, що в них є.
...
В цьому випадку вкладена сітка не має взаємин з батьківською. Як це можна помітити у прикладі, вона не успадкувала {{cssxref("gap")}} предка
...
Елемент `box2` тепер перекривається з `box1`, він виводиться поверх, адже стоїть пізніше в коді.

### Контроль порядку

Порядок, в якому нагромаджуються елементи, можна контролювати за допомогою властивості `z-index` – як із будь-якими розташованими елементами. Можна дати `box2` менший `z-index`, ніж в `box1`, і він буде показаний під `box1`.
