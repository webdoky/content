...
**Синтаксис визначення значення CSS**, формальна граматика, використовується для визначення множини дійсних значень властивості чи функції CSS. На додачу до цього синтаксису, множина дійсних значень може бути звужена семантичними обмеженнями (наприклад, число може бути строго додатним).
...
Ключове слово з наперед визначеним значенням з'являється буквально, без лапок. Наприклад: `auto`, `smaller` чи `ease-in`.
...
У CSS кілька символів можуть з'являтись самі по собі, як то скісна риска ('`/`') чи кома ('`,`'), і використовуватись у визначенні властивості для розділення його частин. Кома нерідко використовується для розділення значень в переліченнях, або параметрах у функціях математичного вигляду; скісна риска нерідко розділяє частини значення, що є семантично відмінними, але мають спільний синтаксис. Зазвичай скісна риска використовується у властивостях-скороченнях; для розділення компонентів, що мають один тип, але належать різним властивостям.
...
- типи даних, що _поділяють ім'я зі властивістю_, поставлене в лапки. У такому випадку тип даних поділяє множину значень зі властивістю. Такі типи нерідко використовуються у визначеннях властивостей-скорочень.
...
- `thin bold 3em`, адже `bold` ставиться поруч з компонентом, визначеним квадратними дужками, і повинно стояти перед ним.
...
Зверніть увагу, що помножувачі не можуть отримати пріоритет над комбінаторами.
...
_Помножувач оклику_ після групи вказує, що група є обов'язковою і мусить дати принаймні одне значення; навіть якщо граматика елементів усередині групи дозволяє пропустити увесь вміст, то принаймні одне значення компонента усе ж повинно бути не пропущено.
