...
Додаються два гнучкі контейнери з однаковою структурою. Вони стилізуються подібно, коли не рахувати їхніх значень `flex-basis`. В кожному контейнері по два безпосередні нащадки: заголовок `<div>` і `<section>`. Елемент `<section>` має дочірній `<div>` зі вмістом, який не заданий як гнучкий елемент, а має задану висоту.
