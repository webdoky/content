...
- `normal`
  ...
  - : Дія цього ключового слова залежить від активного способу компонування:
    ...
    - Для сіткових елементів – призводить до логіки, подібної до логіки `stretch`, окрім блоків зі {{glossary("aspect ratio", "співвідношенням сторін")}} чи природним розміром, для яких це ключове слово поводиться як `start`.
...
Іще є два значення, визначені для флексбоксу, оскільки вони засновані на концепціях [осей гнучкої моделі](/uk/docs/Learn_web_development/Core/CSS_layout/Flexbox#hnuchka-model), то працюють також і при сітковому компонуванні:
