...
> [!NOTE]
> У JavaScript правила, створені за допомогою `@media`, можна отримати за допомогою інтерфейсу {{domxref("CSSMediaRule")}} об'єктної моделі CSS.
...
Значення `<media-query-list>` медіазапиту вміщає значення [`<media-type>`](#typy-media), значення [`<media-feature>`](#mozhlyvosti-media) та [логічні оператори](#lohichni-operatory).

### Типи медіа

_`<media-type>`_ описує загальні категорії пристроїв.
...
> [!NOTE]
> CSS2.1 і [Медіазапити 3](https://drafts.csswg.org/mediaqueries-3/#background) визначали декілька додаткових типів медіа (`tty`, `tv`, `projection`, `handheld`, `braille`, `embossed` і `aural`), але вони стали нерекомендованими в [Медіазапитах 4](https://drafts.csswg.org/mediaqueries/#media-types) і не повинні використовуватися.

### Можливості медіа

_`<media feature>`_ описує конкретні характеристики {{glossary("User agent", "користувацького агента")}}, пристрою виведення чи оточення.
...
- `not`
  ...
  - : Використовується для заперечення медіазапиту, повертаючи `true`, якщо інакше запит повернув би `false`.
    ...
    > [!NOTE]
    > На рівні 3 ключове слово `not` не може використовуватися для заперечення окремого виразу можливості медіа, а лише цілого медіазапиту.
...
- Інтерфейс {{domxref("CSSMediaRule")}}
