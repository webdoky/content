...
> **Примітка:** Підтримка специфічних щодо мов випадків – різна в різних браузерах, тож перевірте [таблицю сумісності з браузерами](#sumisnist-iz-brauzeramy).
...
- `capitalize`
  ...
  - : Ключове слово, котре перетворює кожну першу _літеру_ кожного слова на велику. Решта символів залишається як є (зберігає свій вихідний регістр, вписаний у текст елемента). Під літерою тут розуміється символ, котрий є частиною загальних категорій Unicode {{experimental_inline}} Літери й або Числа; таким чином, будь-які розділові знаки або символи на початку слів – ігноруються.

    > **Примітка:** Не слід очікувати того, що `capitalize` виконуватиме специфічні для мови правила великих літер (як то пропуск артиклів у англійській).

    > **Примітка:** Ключове слово `capitalize` було недостатньо специфіковано в CSS 1 та CSS 2.1. Це призвело до відмінностей між браузерами щодо того, як визначається перша літера (Firefox вважав `-` і `_` літерами, а інші браузери – ні. І WebKit, і Gecko помилково вважали символи на основі літер, як то `ⓐ`, справжніми літерами. Internet Explorer 9 був найближчим до виконання вимог CSS 2, але також мав певні дивні особливості.) Чітко визначивши коректну логіку, специфікація Тексту CSS рівня 3 упорядковує цей безлад. Рядок `capitalize` у таблиці сумісності з браузерами містить версії рушіїв, з яких вони почали підтримувати таку чітко визначену логіку.
