...
Якщо елемент {{HTMLElement("input")}}, {{HTMLElement("select")}} або {{HTMLElement("textarea")}} не має атрибута `autocomplete`, то браузери використовують атрибут `autocomplete` форми-власника елемента, тобто або елемент {{HTMLElement("form")}}, нащадком якого є елемент, або елемент `<form>`, `id` якого вказано в атрибуті елемента [`form`](/uk/docs/Web/HTML/Element/input#form-forma) (див. атрибут `<form>` [`autocomplete`](/uk/docs/Web/HTML/Element/form#autocomplete)).
...
> 3. Щоб форма мала кнопку {{HTMLElement("input/submit", "submit")}}
...
- "`name`"
  ...
  - : Поле очікує на значення, що є повним іменем особи. Як правило, краще використовувати "`name`", ніж розбивати ім'я на складові частини, тому що такий підхід уникає проблеми розмаїття людських імен та того, як воно структуруються; проте також можна використовувати наступні значення `autocomplete`, якщо все ж потрібно розбити ім'я на складові частини:
    ...
    - "`honorific-prefix`"
      ...
      - : Префікс або титул, наприклад, "Пані", "Пан", "Панна", "Місис", "Міс", "Містер", "Доктор", "Мадемуазель".
    - "`given-name`"
...
- "`one-time-code`"
  - : Одноразовий пароль (OTP), що використовується для перевірки інформації про користувача, найчастіше – його номера телефону як додаткового чинника процесу автентифікації.
- "`organization-title`"
