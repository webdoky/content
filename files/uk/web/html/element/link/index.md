...
Цей приклад вказує шлях до таблиці стилів у атрибуті `href`, а також атрибут [`rel`](/uk/docs/Web/HTML/Attributes/rel) зі значенням `stylesheet`. `rel` означає "relationship" – "взаємини", і це одна з ключових можливостей елемента `<link>`: значення вказує на те, як саме зовнішній ресурс пов'язаний з документом.
...
- `fetchpriority`
  ...
  - : Надає підказку щодо відносного пріоритету при отриманні ресурсу певного типу.
    Дозволені значення:

    - `high`
      - : Отримує ресурс із високим пріоритетом відносно інших ресурсів того самого типу.
    - `low`
      - : Отримує ресурс із низьким пріоритетом відносно інших ресурсів того самого типу.
    - `auto`
    ...
    Докладніше в {{domxref("HTMLLinkElement.fetchPriority")}}.
...
- `imagesizes`
  - : Обмежений наявністю атрибутів `rel="preload"` і `as="image"`, атрибут `imagesizes` має синтаксис і семантику, подібні до атрибута [`sizes`](/uk/docs/Web/HTML/Element/img#sizes), котрий вказує завантажувати наперед відповідний ресурс, використаний елементом `img`, що має відповідні значення атрибутів `srcset` і `sizes`.
- `imagesrcset`
  - : Обмежений наявністю атрибутів `rel="preload"` і `as="image"`, атрибут `imagesrcset` має синтаксис і семантику, подібні до атрибута [`srcset`](/uk/docs/Web/HTML/Element/img#srcset), котрий вказує завантажувати наперед відповідний ресурс, використаний елементом `img`, що має відповідні значення атрибутів `srcset` і `sizes`.
- `integrity`
