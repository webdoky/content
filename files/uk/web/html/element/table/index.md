...
Наступні елементи є частиною структури таблиці:
...
Рамка `<table>` утворює табличний контекст форматування. Елементи всередині `<table>` породжують прямокутні рамки. Кожна рамка займає певне число комірок таблиці, згідно з наступними правилами:

1. Рамки рядів заповнюють таблицю в порядку своєї появи в коді, згори донизу. Кожна рамка ряду займає один ряд комірок.
2. Рамка групи рядів займає один або більше рядів комірок.
3. Рамки колонок розміщаються поруч одна з одною, в порядку своєї появи в коді. Залежно від значення атрибута [`dir`](/uk/docs/Web/HTML/Global_attributes/dir), колонки розміщуються зліва направо або справа наліво. Кожна рамка колонки займає одну або більше колонок комірок.
4. Рамка групи колонок займає одну або більше колонок комірок.
5. Рамка комірки може простягатися на кілька рядів і колонок. Користувацькі агенти обрізають комірки, щоб вони вмістилися у доступну кількість рядів і колонок.
...
Задля потреб оформлення елементи таблиці можна розглядати як розміщені на шести накладених один на одного шарах:

![Шари табличних елементів](table_element_layers.png)

Фон, заданий на елементі в одному з шарів, буде помітний лише тоді, коли шари, що знаходяться над ним, матимуть прозорий фон. Пропущена комірка візуалізується так, ніби її місце зайнята анонімною коміркою.
