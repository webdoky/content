...
Те, як `<input>` працює, значною мірою залежить від значення його атрибута [`type`](#type-typ), – тому різні типи покриті на власних довідкових сторінках. Якщо цей атрибут не вказаний – використовується усталене значення `text`.
...
<table class="no-markdown">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Подібний до <code>:invalid</code>, але активується при втраті фокуса. Збігається з невалідними полями введення, але лише після взаємодії з ними користувача. Наприклад, коли користувач перевів фокус такому контрольному елементові, а потім залишив його, або спробував подати форму з невалідним контрольним елементом.
      </td>
    </tr>
  </tbody>
</table>
