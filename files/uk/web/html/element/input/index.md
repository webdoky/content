...
Те, як `<input>` працює, значною мірою залежить від значення його атрибута [`type`](#type), – тому різні типи покриті на власних довідкових сторінках. Якщо цей атрибут не вказаний – використовується усталене значення `text`.
...
<table class="no-markdown">
  ...
  <tbody>
    <tr>
      ...
      <td>
        Натисна кнопка без усталеної поведінки, що показує значення атрибута <a href="#attr-value"><code>value</code></a>, усталено – пуста.
      </td>
      <td id="examplebutton">
...
&#x3C;input type="email" name="email"/></pre>
    ...
    <tr>
      ...
      <td>
        ...
        Для встановлення типів файлів, які може обрати цей елемент, слід використовувати атрибут <a href="#attr-accept"><code>accept</code></a>.
      </td>
      <td id="examplefile">
        <pre class="brush: html hidden">
...
&#x3C;input id="userId" name="userId" type="hidden" value="abc123"></pre>
    ...
    <tr>
      ...
      <td>
        Графічна кнопка <code>submit</code> (подання). Показує зображення, встановлене атрибутом <a href="#attr-src"><code>src</code></a>.
        Вміст атрибута <a href="#attr-alt"><code>alt</code></a>
        показується, якщо зображення з атрибута <code>src</code> відсутнє.
      </td>
      <td id="exampleimage">
        <pre class="brush: html hidden">
...
&#x3C;input type="number" name="number"/></pre>
    ...
    <tr>
      ...
      <td>
        ...
        Попередить користувача, якщо сайт не є безпечним.
      </td>
...
&#x3C;input type="password" name="password"/></pre>
    ...
    <tr>
      ...
      <td>
        Радіокнопка, що дає змогу обрати одне значення серед кількох варіантів з однаковим значенням атрибута <a href="#attr-name"><code>name</code></a>.
      </td>
      <td id="exampleradio">
        <pre class="brush: html hidden">
...
| [`autocomplete`](#autocomplete)     | усі                              | Вказівка для функціональності автоматичного заповнення форм                                                        |
| [`capture`](#capture)               | file                             | Спосіб захоплення медіа для контрольних елементів завантаження файлів                                              |
| [`checked`](#checked)               | radio, checkbox                  | Вказує, чи є контрольний елемент обраним                                                                           |
| [`dirname`](#dirname)               | text, search                     | Ім‘я поля форми для надсилання напряму письма при поданні форми                                                    |
| [`disabled`](#disabled)             | усі                              | Вказує, чи є контрольний елемент вимкненим                                                                         |
| [`form`](#form)                     | усі                              | Асоціює контрольний елемент з елементом форми                                                                      |
| [`formaction`](#formaction)         | image, submit                    | URL для подання форми                                                                                              |
| [`formenctype`](#formenctype)       | image, submit                    | Тип кодування даних для подання форми                                                                              |
| [`formmethod`](#formmethod)         | image, submit                    | Метод HTTP для подання форми                                                                                       |
| [`formnovalidate`](#formnovalidate) | image, submit                    | Вказівка обходити валідацію контрольного елемента при поданні форми                                                |
| [`formtarget`](#formtarget)         | image, submit                    | Контекст перегляду для подання форми                                                                               |
| [`height`](#height)                 | image                            | Те саме, що атрибут height для {{htmlelement('img')}}; вертикальний розмір                                         |
| [`list`](#list)                     | майже усі                        | Значення атрибута `id` елемента {{htmlelement('datalist')}} із варіантами автозаповнення                           |
| [`max`](#max)                       | числові типи                     | Найбільше прийнятне значення                                                                                       |
| [`maxlength`](#maxlength)           | password, search, tel, text, url | Найбільша прийнятна довжина (кількість символів) <code>value</code>                                                |
| [`min`](#min)                       | числові типи                     | Найменше прийнятне значення                                                                                        |
| [`minlength`](#minlength)           | password, search, tel, text, url | Найменша прийнятна довжина (кількість символів) <code>value</code>                                                 |
| [`multiple`](#multiple)             | email, file                      | Булеве значення. Вказівка приймати кілька значень                                                                  |
| [`name`](#name)                     | усі                              | Ім‘я контрольного елемента. Подається разом із формою як частина пари ім‘я-значення.                               |
| [`pattern`](#pattern)               | password, text, tel              | Схема, котрій мусить відповідати значення <code>value</code>, щоб бути дійсним                                     |
| [`placeholder`](#placeholder)       | password, search, tel, text, url | Текст, що з‘являється у контрольному елементі, коли в ньому немає жодного значення                                 |
| [`readonly`](#readonly)             | майже усі                        | Булеве значення. Вказівка заборонити зміну значення                                                                |
| [`required`](#required)             | майже усі                        | Булеве значення. Значення повинно бути вказане, або прапорець мусить бути поставлений, щоб форму можна було подати |
| [`size`](#size)                     | email, password, tel, text, url  | Розмір контрольного елемента                                                                                       |
| [`src`](#src)                       | image                            | Те саме, що атрибут <code>src</code> елемента {{htmlelement('img')}}; адреса ресурсу зображення                    |
| [`step`](#step)                     | числові типи                     | Крок квантування дійсних значень.                                                                                  |
| [`type`](#type)                     | усі                              | Тип контрольного елемента                                                                                          |
| [`value`](#value)                   | усі                              | Початкове значення контрольного елемента.                                                                          |
| [`width`](#width)                   | image                            | Те саме, що атрибут <code>width</code> елемента {{htmlelement('img')}}                                             |
...
- `alt` (альтернатива)
  ...
  - : Дійсний лише для кнопки `image`. Атрибут `alt` задає альтернативний текст для кнопки, що буде показаний, якщо зображення {{htmlattrxref("src", "input", "", 1)}} немає або його не виходить завантажити. Дивіться тип поля введення {{HTMLElement("input/image", "image")}}.
...
- `form` (форма)
  ...
  - : Рядок, що вказує елемент {{HTMLElement("form")}}, з котрим поле введення асоційоване (тобто його **форму-господаря**). Значення такого рядка, якщо є, мусить збігатися зі значенням атрибута [`id`](#id) елемента `<form>`, присутнього у тому ж документі. Якщо цей атрибут не вказаний, то елемент `<input>` асоціюється із найближчою формою-предком, якщо така є.
...
- `list` (список)
  ...
  - : Значення, встановлене для атрибута `list`, повинно бути значенням атрибута {{domxref("Element.id", "id")}} елемента {{HTMLElement("datalist")}}, присутнього в тому ж документі. `<datalist>` надає список наперед визначених значень, щоб пропонувати їх користувачеві для відповідного поля введення. Усі значення у списку, що є несумісними із [`type`](#type), не включаються у запропоновані варіанти. Надані значення є пропозиціями, а не обмеженням: користувачі можуть обирати із наданого списку або ввести інше значення.
...
- `max` (максимум)
  ...
  - : Чинний для `date`, `month`, `week`, `time`, `datetime-local`, `number` та`range`. Встановлює найбільше значення діапазону дозволених значень. Якщо введене у поле значення [`value`](#value) більше за значення цього атрибута, то елемент провалює [валідацію обмежень](/uk/docs/Web/Guide/HTML/Constraint_validation). Якщо значення атрибута `max` не є числом, то елемент не має максимального значення.
...
- `name` (ім‘я)
  ...
  - : Рядок, що вказує ім‘я контрольного елемента. Це ім‘я подається разом зі значенням елемента при поданні форми.
    ...
    Атрибут [`name`](#name) створює особливу поведінку на радіокнопках.
...
- `step` (крок)
  ...
  - : Чинний для числових типів полів введення, включаючи `number`, типи дати та часу та `range`. Атрибут [`step`](/uk/docs/Web/HTML/Attributes/step) – це число, що вказує гранулярність, котрої мусить дотримуватись значення.
    ...
    Значення мусить бути додатним числом, цілим чи з рухомою комою – або особливим значенням `any`, котре вказує не застосовувати крокування і приймати будь-яке значення (прийнятне згідно з іншими обмеженнями, наприклад, [`min`](#min) і [`max`](#max).

    Якщо явно не вказане значення `any`, то дійсні значення для типів `number`, дати й часу, а також `range` охоплюють: основу крокування – значення [`min`](#min), та всі її зростання на розмір кроку, аж до значення [`max`](#max) (якщо воно вказане).
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <a href="#step"><code>step</code></a> і <a href="#max"><code>max</code></a>.
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        встановленому атрибутами <a href="#min"><code>min</code></a> і <a href="#max"><code>max</code></a>, і з кроком, встановленим атрибутом <a href="#step"><code>step</code></a>.
      </td>
    </tr>
    <tr>
      ...
      <td>
        ...
        встановленими атрибутами <a href="#min"><code>min</code></a>
        і <a href="#max"><code>max</code></a>, або
...
> **Примітка:** Слід уникати використання атрибута [`placeholder`](#placeholder), коли це можливо. Якщо необхідно підписати елемент `<input>`, то слід використовувати елемент {{HTMLElement("label")}}.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        <a href="/uk/docs/Web/Guide/HTML/Content_categories#phrasing_content">оповідальний вміст</a>. Якщо <a href="#type"><code>type</code></a> – не
...
- [Хутка проба: Великі мішені дотику – Проєкт A11Y (англ.)](https://www.a11yproject.com/posts/large-touch-targets/)
