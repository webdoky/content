...
> [!NOTE]
> Користувач може погратися з HTML за лаштунками, тому сайт _не мусить_ використовувати валідацію на клієнтському боці для потреб безпеки. Розробник _мусить_ перевірити на боці сервера будь-яку транзакцію, в котрій надане значення може мати будь-якого роду безпекові наслідки.
...
{{EmbedLiveSample('bazove-chyslove-pole', 600, 40)}}
...
Після оголошення кількох змінних до `button` додається слухач повідомлень для контролю механізму перемикання. Це полягає в змінах атрибута `class` і елемента {{HTMLElement("label")}}, а також оновленні виведених значень у двох наборах полів, коли натиснена кнопка.
