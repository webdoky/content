...
Атрибут [`value`](/uk/docs/Web/HTML/Element/input#value) – це рядок, що містить поточне значення тексту, введеного в текстове поле. Його можна отримати в JavaScript за допомогою властивості {{domxref("HTMLInputElement")}} `value`.
...
Максимальна кількість символів (як кодових одиниць UTF-16), котру користувач може ввести в поле `text`. Це повинно бути ціле числове значення, 0 або більше. Якщо не задано `maxlength`, або якщо задано недійсне значення, то поле `text` не має максимальної довжини. Це значення також повинно бути більшим або рівним значенню `minlength`.
...
Мінімальна кількість символів (як кодових одиниць UTF-16), котру користувач може ввести в поле `text`. Це повинно бути невід'ємне ціле числове значення, менше або рівне значенню, заданому `maxlength`. Якщо не задано `minlength`, або якщо задано недійсне значення, то пошукове поле не має мінімальної довжини.
...
При поданні пара даних ім'я-значення буде `name=Chris` (якщо перед поданням як значення поля було введено "Chris"). Необхідно завжди пам'ятати додавати до елемента {{HTMLElement("input")}} атрибут [`name`](/uk/docs/Web/HTML/Element/input#name): інакше значення текстового поля не буде включено в подані дані.

### Задання заповнювачів

Всередині текстового поля можна дати змістовний заповнювач, котрий служитиме підказкою щодо того, що з полем робити, за допомогою атрибута [`placeholder`](/uk/docs/Web/HTML/Element/input#placeholder). Погляньте на наступний приклад:
...
Мінімальну довжину (в символах) уведеного значення можна задати за допомогою атрибута [`minlength`](/uk/docs/Web/HTML/Element/input#minlength); подібно до цього – є [`maxlength`](/uk/docs/Web/HTML/Element/input#maxlength) для задання максимальної довжини, так само в символах.
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <a href="/uk/docs/Web/HTML/Element/input#required"><code>required</code></a> і
        <a href="/uk/docs/Web/HTML/Element/input#size"><code>size</code></a>
      </td>
    </tr>
    <tr>
      <td><strong>Атрибути IDL</strong></td>
      <td><a href="/uk/docs/Web/HTML/Element/input#list"><code>list</code></a>, <code>value</code></td>
    </tr>
    <tr>
