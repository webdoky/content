...
У випадку радіокнопки, на якій заданий атрибут [`required`](/uk/docs/Web/HTML/Attributes/required), або однойменної групи радіокнопок, у якій щонайменше на одній з кнопок є `required`, радіокнопка повинна бути вибрана, щоб такий контрольний елемент вважався дійсним. Якщо жодна з радіокнопок не позначена, під час валідації властивість [`valueMissing`](/uk/docs/Web/API/ValidityState/valueMissing) об'єкта {{domxref("ValidityState")}} поверне під час валідації `true`, і браузер попросить користувача обрати один з варіантів.
