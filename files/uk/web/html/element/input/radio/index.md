...
Радіокнопками їх звуть, тому що вони схожі на вигляд і працюють схоже на натисні кнопки в старих радіоприймачах, таких, як зображений нижче.
...
Радіогрупа окреслюється шляхом задання кожній радіокнопці групи однакового значення атрибута {{htmlattrxref("name", "input")}}. Щойно задана радіогрупа, вибір будь-якої радіокнопки з цієї групи автоматично скасовуватиме вибір будь-якої до того вибраної радіокнопки тієї ж групи.
...
Тут три радіокнопки, кожна з яких має атрибут `name` зі значенням `contact`, але всі вони – з унікальними значеннями `value`, що унікально вказують на певну радіокнопку групи. Також вони мають унікальні {{domxref("Element.id", "id")}}, що їх використовує атрибут {{htmlattrxref("for", "label")}} елемента {{HTMLElement("label")}}, аби пов‘язати позначки з радіокнопками.
