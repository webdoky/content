...
> **Примітка:** Хоч елементи `<input>` типу `button` досі є цілковито дійсним HTML, та бажаним способом створювати кнопки є новіший елемент {{HTMLElement("button")}}. Враховуючи, що текст підпису {{HTMLElement("button")}} вставляється між початковим і кінцевим тегами, можна включити в підпис HTML, навіть зображення.
...
{{EmbedLiveSample("dodavannia-do-knopok-modyfikatsii-klavish", 650, 100)}}
...
Якщо атрибут `disabled` не заданий, то кнопка успадковує свій стан `disabled` від свого елемента-предка. Це уможливлює ввімкнення й вимкнення груп елементів водночас, шляхом загортання їх у контейнер, наприклад, елемент {{HTMLElement("fieldset")}}, а потім задання `disabled` на контейнері.
