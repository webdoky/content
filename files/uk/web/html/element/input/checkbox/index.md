...
> **Примітка:** Якщо поле для галочки пусте при подачі форми, то на сервер не подаються ані назва, ані значення. Не існує суто HTML метода подати непозначений стан поля для галочки (виду `value=unchecked`). При потребі подати усталене значення поля, коли воно пусте, можна включити JavaScript для створення всередині форми {{HTMLElement("input/hidden", '&lt;input type="hidden"&gt;')}} зі значенням, що позначає пустий стан.
...
- `checked` (перевірений)
  ...
  - : [Булів](/uk/docs/Glossary/Boolean/HTML) атрибут, що вказує, чи має поле галочку спочатку (при завантаженні сторінки). Воно _не_ вказує, чи є поле порожнім наразі: якщо стан поля зміниться, цей атрибут вмісту не відобразить змін. (Оновиться лише IDL атрибут {{domxref("HTMLInputElement")}}'а `checked`ʼ.)
