...
У браузерах, що не підтримують полів `month`, цей елемент доладно відступає до [`<input type="text">`](/uk/docs/Web/HTML/Element/input/text), а іще може бути автоматична валідація введеного тексту, аби пересвідчитись, що він має очікуваний формат.
...
Найбазовіше використання `<input type="month">` включає базову комбінацію елементів {{HTMLElement("input")}} і {{htmlelement("label")}}, як це видно нижче:
