...
Вигляд цього контрольного елемента, загалом, відрізняється від браузера до браузера; наразі підтримка є випадковою: лише Chrome, Opera й Edge на настільних комп'ютерах, а також сучасні версії більшості мобільних браузерів мають робочі реалізації.
...
Як наслідок, браузери, що це підтримують, покажуть помилку, якщо спробувати подати дату, що лежить поза заданими межами, або порожнє її поле.
...
Якщо спробувати подати форму, не задавши і місяць, і рік (або з датою поза заданими межами), то браузер покаже помилку.
...
Один зі способів це обійти – додати полю `month` атрибут [`pattern`](/uk/docs/Web/HTML/Element/input#pattern) `month`.
...
Код JavaScript, що обробляє вибір конкретного підходу й налаштовує список років для виводу в ненативному `<select>` року – нижче..
