...
- `src`
  ...
  > **Примітка:** Сторінка `about:blank` використовує URL документа, щ вбудовується, як свій базовий URL, коли розв'язуються відносні URL, наприклад, якірні посилання.

- `srcdoc`

  - : Супутній HTML для вбудовування, що відкидає атрибут `src`. Його вміст повинен відповідати синтаксису цілого документа URL, що складається з директиви doctype, тегів `<html>`, `<body>` тощо, хоч і більшість з цих тегів можна пропустити, залишивши лише вміст тіла. Такий документ матиме адресу `about:srcdoc`. Якщо браузер не підтримує атрибут `srcdoc`, то звернеться до URL у атрибуті `src`.

    > **Примітка:** Сторінка `about:srcdoc` використовує URL документа, щ вбудовується, як свій базовий URL, коли розв'язуються відносні URL, наприклад, якірні посилання.
...
Цей приклад безпосередньо візуалізує вихідний код у супутньому фреймі. Це можна використовувати як техніку для запобігання ін'єкціям сценаріїв при виведенні вмісту, створеного користувачами, коли поєднати це з атрибутом `sandbox`.

Зверніть увагу на те, що при використанні `srcdoc` будь-які відносні URL у вбудованому вмісті розв'язуються відносно URL вбудованої сторінки. При потребі використання якірних посилань, що вказують на місця у вбудованому вмісті, необхідно явно задати `about:srcdoc` як базовий URL.
...
- `&lt;` і `<` у атрибуті `srcdoc` представляють один і той же символ. Таким чином, щоб зробити таку послідовність справжньою послідовністю екранування в документі HTML, замініть усі амперсанди (`&`) на `&amp;`. Наприклад, `&lt;` стане `&amp;lt;`, а `&amp;` стане `&amp;amp;`.
