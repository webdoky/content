---
title: "<label>: Елемент підпису поля введення"
...
Пов'язання `<label>` з елементом {{htmlelement("input")}} пропонує доволі суттєві переваги:
...
Щоб пов'язати `<label>` з елементом `<input>`, треба надати `<input>` атрибут `id`. Далі `<label>` потрібен атрибут `for`, чиє значення – таке ж, як в атрибута `id` поля.
...
### Простий приклад з підписом

```html
<label>Клацни мене <input type="text" /></label>
```

{{EmbedLiveSample('prostyi-pryklad-z-pidpysom', '200', '50', '')}}
...
{{EmbedLiveSample('vykorystannia-atrybuta-for', '200', '50', '')}}
...
Якщо [форма](/uk/docs/Web/HTML/Element/form) чи розділ форми потребує заголовку, слід використовувати елемент {{HTMLElement("legend")}}, розташований всередині {{HTMLElement("fieldset")}}.
