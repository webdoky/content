...
- `sizes`
  ...
  - : Один чи більше рядків, розділених комами, що вказують набір розмірів джерела. Кожен розмір джерела складається із:
    ...
    Значення розміру джерела може бути будь-якою невід'ємною [довжиною](/uk/docs/Web/CSS/length). Воно не повинно використовувати функції CSS, крім [математичних](/uk/docs/Web/CSS/CSS_Values_and_Units/CSS_Value_Functions#matematychni-funktsii). Одиниці вимірювання тлумачаться так само, як [медійні запити](/uk/docs/Web/CSS/CSS_media_queries), тобто всі відносні одиниці довжини є відносними щодо кореня документа, а не елемента `<img>`, і значення `em` є відносним щодо розміру шрифту в корені, а не розміру шрифту зображення. [Відсоткові](/uk/docs/Web/CSS/percentage) значення не допускаються.

    Крім цього, можна скористатися значенням `auto`, замінивши ним весь список розмірів або перший запис у списку. Це валідно лише в поєднанні з `loading="lazy"` і розв'язується до [конкретного розміру](/uk/docs/Web/CSS/image) зображення.
