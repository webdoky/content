...
- Атрибут `alt` зберігає текстовий опис зображення, котрий не є обов'язковим, але є **приголомшливо корисним** для доступності: читачі з екрана читають своїм користувачам цей опис, щоб вони знали, що означає зображення. Альтернативний текст також виводиться на сторінці, якщо зображення з якоїсь причини не може бути завантажене: наприклад, через мережеві помилки, блокування вмісту чи гниття посилань.

Є чимало інших атрибутів для досягнення різних цілей:

- Контроль [посилача](/uk/docs/Web/HTTP/Headers/Referrer-Policy)/{{glossary("CORS")}} для безпеки й приватності: дивіться {{htmlattrxref("crossorigin", "img")}} й {{htmlattrxref("referrerpolicy", "img")}}.
- Щоб задати зображенню його природний розмір, слід задати й атрибут {{htmlattrxref("width", "img")}}, і атрибут {{htmlattrxref("height", "img")}}: це дасть змогу зарезервувати для зображення місце й уникнути зсувів компонування вмісту.
- Підказки чуйності зображень за допомогою {{htmlattrxref("sizes", "img")}} і {{htmlattrxref("srcset", "img")}} (також дивіться елемент {{htmlelement("picture")}} і наш підручник [Чуйні зображення](/uk/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images)).
...
Якщо помилка трапляється при завантаженні чи візуалізації помилки, і якщо був встановлений обробник помилки {{htmlattrxref("onerror")}} для події {{domxref("Element/error_event", "error")}}, то такий обробник помилки буде викликаний. Це може статися в низці ситуацій, в тому числі:
...
- {{htmlattrdef("alt")}}
  ...
  - : Визначає альтернативний текст – опис зображення.
...
- {{htmlattrdef("decoding")}}
  ...
  - : Надає браузерові підказку щодо розшифровування зображення. Дозволені значення:

    - `sync`
      - : Розшифрувати зображення синхронно, для неподільного представлення поруч з рештою вмісту.
    - `async`
...
- {{htmlattrdef("ismap")}}
  ...
  - : Цей булів атрибут вказує, що зображення є частиною [серверної карти](https://en.wikipedia.org/wiki/Image_map#servernyi-bik). Якщо це так, то координати, в яких користувач клацнув зображення, надсилаються на сервер.

    > **Примітка:** Цей атрибут дозволений лише тоді, коли елемент `<img>` є нащадком елемента {{htmlelement("a")}} з дійсним атрибутом {{htmlattrxref("href","a")}}. Це надає користувачам без вказівних пристроїв запасний напрям переходу.

- {{htmlattrdef("loading")}}

  - : Вказує, як браузер повинен завантажувати зображення:

    - `eager`: Завантажує зображення негайно, незалежно від того, чи знаходиться зображення наразі в межах області перегляду (це значення – усталене).
    - `lazy`: Відкладає завантаження зображення, поки воно не досягне обчисленої відстані від області перегляду, визначеної браузером. Задумом є уникнення використання мережевого з'єднання й пропускної здатності сховища, необхідних для обробки зображення, поки не настане певність, що воно знадобиться. Це загалом покращує швидкодію вмісту в найтиповіших ситуаціях.
...
- {{htmlattrdef("sizes")}}
  ...
  - : Один чи більше рядків, розділених комами, що вказують набір розмірів джерела. Кожен розмір джерела складається із:
    ...
    1. [Умови медіа](/uk/docs/Web/CSS/Media_Queries/Using_media_queries#syntaksys). Повинна бути опущена для останнього елемента списку.
...
- {{htmlattrdef("longdesc")}} {{deprecated_inline}}
  ...
  - : Посилання на більш докладний опис зображення. Прийнятними значеннями є {{glossary("URL")}} і {{htmlattrxref("id")}} елемента.
...
- {{htmlattrdef("name")}} {{deprecated_inline}}
  - : Ім'я елемента. Натомість слід використовувати атрибут {{htmlattrxref("id")}}.
...
```html
<img src="favicon144.png" alt="Логотип ВебДоків" />
```
...
```html
<img src="favicon72.png" alt="Логотип ВебДоків" srcset="favicon144.png 2x" />
```
...
Значення атрибута `alt` повинно ясно й стисло описувати вміст зображення. Воно не повинно описувати присутність самого зображення чи ім'я файлу зображення. Якщо атрибут `alt` навмисно упущений через те, що зображення не має текстового еквівалента, слід розглянути варіанти представлення того, що зображення намагається повідомити, в інший спосіб.
  ...
  <tbody>
    <tr>
      ...
      <td>
        ...
        >,
        <a href="/uk/docs/Web/Guide/HTML/Content_categories#embedded_content"
          >вбудований вміст</a
        >,
        <a href="/uk/docs/Web/Guide/HTML/Content_categories#palpable_content"
          >дотиковий вміст</a
