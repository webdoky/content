...
Якщо помилка трапляється при завантаженні чи візуалізації зображення, і якщо був встановлений обробник помилки `onerror` для події {{domxref("Element/error_event", "error")}}, то такий обробник помилки буде викликаний. Це може статися в декількох ситуаціях, в тому числі:
...
Атрибут [`title`](/uk/docs/Web/HTML/Global_attributes#title) не є прийнятною заміною атрибута `alt`. Крім того, слід уникати дублювання значення атрибута `alt` в атрибуті `title`, оголошеному на тому самому зображенні. Це призвело б до того, що певні читачі з екрана оголосили б опис двічі, спантеличуючи користувачів.
