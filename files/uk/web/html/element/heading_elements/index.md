...
### Задання одноманітного розміру шрифту для `<h1>`

[Стандарт HTML](https://html.spec.whatwg.org/multipage/rendering.html#sections-and-headings) вимагає, щоб елементи `<h1>` в елементах `<section>`, `<article>`, `<aside>` і `<nav>` відображалися як `<h2>` (з меншим {{cssxref("font-size")}} та інакшим {{cssxref("margin-block")}}), або як `<h3>`, якщо вкладені на ще один рівень, і так далі.

> [!NOTE]
> Існує [пропозиція](https://github.com/whatwg/html/issues/7867) вилучити цей заданий усталений стиль, щоб `<h1>` завжди мали однаковий усталений стиль. Ця пропозиція наразі [реалізована в Firefox Nightly](/uk/docs/Mozilla/Firefox/Experimental_features#ua_styles_for_h1_nested_in_sectioning_elements).

Аби забезпечити сталість візуалізації `<h1>`, слід скористатися таким стилістичним правилом:
...
Інакший варіант, що дає змогу уникнути переписування інших стилістичних правил, що націлюються на `<h1>`, – використання {{cssxref(":where()")}}, що дає нульову специфічність:
