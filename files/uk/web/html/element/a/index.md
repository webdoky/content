...
Елемент [HTML](/uk/docs/Web/HTML) **`<a>`** (він же елемент _якоря_) з [атрибутом `href`](#href) створює гіперпосилання до вебсторінок, файлів, адрес електронної пошти, місць на тій самій сторінці чи будь-чого іще, на що може посилатися URL.
...
- `href`
  ...
  - : URL, на котрий вказує гіперпосилання. Посилання не обмежені HTTP URL – вони можуть використовувати будь-яку схему URL, котру підтримують браузери:
    ...
    - Виконуваний код з [URL `javascript:`](/uk/docs/Web/URI/Reference/Schemes/javascript)
    ...
    Понад це, інші можливості URL можуть задавати конкретні частини ресурсу, серед яких:

    - Розділи сторінки – за допомогою фрагментів документа
    - Конкретні ділянки тексту – за допомогою [текстових фрагментів](/uk/docs/Web/URI/Reference/Fragment/Text_fragments)
    - Шматочки медіафайлів – за допомогою медійних фрагментів
...
- `charset` {{Deprecated_Inline}}
  ...
  - : Підказка щодо {{Glossary("character encoding", "кодування символів")}} ресурсу за URL.
    ...
    > [!NOTE]
    > Цей атрибут є нерекомендованим і **не повинен використовуватись розробниками**. Використовуйте HTTP заголовок {{HTTPHeader("Content-Type")}} за вказаним URL.
...
Якірними елементами часто зловживають у вигляді підробних кнопок, задаючи їх атрибут `href` зі значенням `#` чи [`javascript:void(0)`](/uk/docs/Web/URI/Reference/Schemes/javascript), щоб запобігти оновленню сторінки, а потім слухаючи їхні події `click`.
