...
- `dirname`
  ...
  - : Цей атрибут вживається для задання напряму письма тексту елемента в спосіб, подібний до атрибута [`dirname`](/uk/docs/Web/HTML/Element/input#dirname-imia-napriamu) елемента `<input>`. Серед можливих значень: `ltr` – зліва направо, `rtl` – справа наліво. Усталене значення – `ltr`.
    Значення цього атрибута надсилається в форматі `{elementName}.dir={value}`, де `{elementName}` – ім'я елемента `<textarea>`, а `{value}` може бути `ltr` або `rtl`.
