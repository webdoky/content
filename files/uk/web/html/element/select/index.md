...
Також можна скористатися псевдокласом {{cssxref(":open")}}, щоб оформити елемент `<select>` у відкритому стані, тобто коли показано спадний список варіантів. Це не стосується багаторядкових елементів `<select>` (тобто тих, що мають атрибут [`multiple`](/uk/docs/Web/HTML/Attributes/multiple)) – вони мають схильність візуалізуватися як прокрутна рамка-список, а не спадний список, тому не мають стану відкритості.
