---
title: <datalist> – елемент списку даних HTML
...
> [!NOTE]
>
> `<datalist>` не є заміною для {{HTMLElement("select")}}. `<datalist>`, по суті, не є полем для введення; це список пропонованих значень для пов'язаного контрольного елемента. Цей контрольний елемент все одно може прийняти будь-яке значення, що проходить валідацію, навіть якщо його нема в списку пропозицій.
...
> [!NOTE]
> Коли ці типи не підтримуються, замість них візуалізується базовий тип `text`, що утворює текстове поле. Таке поле коректно розуміє рекомендовані значення та виводить їх користувачам у спадному меню.
