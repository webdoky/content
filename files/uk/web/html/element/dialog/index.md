...
- Властивість {{cssxref("transition-behavior")}}
  - : Задайте на переходах `display` і `overlay` `transition-behavior: allow-discrete` (або на скороченні {{cssxref("transition")}}), щоб увімкнути дискретні переходи на цих двох властивостях, які усталено не піддаються анімуванню.
