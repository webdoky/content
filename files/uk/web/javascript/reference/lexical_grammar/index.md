...
| Інші         | Інші пробільні символи Unicode       | \<USP>      | [Символи загальної категорії "Space_Separator"][nabir-probilnykh-rozdiliuvachiv]                    |                           |
...
Коментарі використовуються до коду JavaScript для додавання підказок, нотаток, пропозицій чи застережень. Вони можуть робити цей код легшим для прочитання та розуміння. Вони можуть використовуватись для вимикання коду та запобігання його виконанню; це може бути цінним інструментом зневадження.
...
На додачу до цього, JavaScript дозволяє використання в ідентифікаторах [послідовностей екранування Unicode](#poslidovnosti-ekranuvannia-unicode), у вигляді `\u0000` або `\u{000000}`, що кодує ті самі рядкові значення, що й фактичні символи Unicode. Наприклад, `你好` і `\u4f60\u597d` – це один і той же ідентифікатор:
...
Не у всіх місцях приймається повний діапазон ідентифікаторів. Частина записів, як то оголошення функцій, вирази функцій та оголошення змінних вимагають використання ідентифікаторів, що не є [зарезервованими словами](#zarezervovani-slova).

```js example-bad
function import() {} // Заборонено: import – зарезервоване слово.
```

Перш за все, приватні властивості та властивості об'єктів – дозволяють зарезервовані слова.

```js
const obj = { import: "value" }; // Дозволено, попри те, що `import` – зарезервоване
class C {
  #import = "value";
}
```
...
#### Майбутні зарезервовані слова в давніших стандартах
...
Літерали BigInt не можуть починатися на `0` у зв'язку з униканням плутанини з історичними вісімковими літералами.
...
[Рядковий](/uk/docs/Web/JavaScript/Data_structures#typ-string) – це нуль або більше кодових точок Unicode, загорнутих в одинарні чи подвійні лапки. Також кодові точки Unicode можуть бути представлені у вигляді власних послідовностей екранування. В рядкових літералах можуть зустрічатися в буквальному вигляді усі кодові точки, окрім оцих кодових точок, що закривають лапки:
...
Тут [`++`](/uk/docs/Web/JavaScript/Reference/Operators/Increment) не розглядається як постфіксний оператор, що застосовується до змінної `b`, тому що між `b` і `++` зустрічається символ кінця рядка.
