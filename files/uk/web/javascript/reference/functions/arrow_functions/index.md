...
У прикладі вище і дужки навколо параметра, і фігурні дужки навколо тіла функції – можуть бути опущені. Проте – не лише за певних умов.
...
У стислому тілі задається лише один вираз, котрий стає неявним поверненим значенням. У блоковому тілі слід використовувати явну інструкцію `return`.
...
Так відбувається через те, що JavaScript впізнає стрілкову функцію як таку, що має стисле тіло, лише якщо знак після стрілки не є лівою фігурною дужкою, тож код всередині фігурних дужок обробляється як послідовність інструкцій, у якій `foo` є [позначкою](/uk/docs/Web/JavaScript/Reference/Statements/label), а не ключем літерала об'єкта.
