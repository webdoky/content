...
Для деструктурування, як об'єктів, так і масивів, є два типи патернів деструктурування: _патерн зв'язування_ й _патерн присвоєння_, що мають дещо відмінні синтаксиси.
...
>
> `{ a, b } = { a: 1, b: 2 }` не є дійсним синтаксисом сам по собі, оскільки `{a, b}` зліва вважається блоком коду, а не літералом об'єкта. Проте `({ a, b } = { a: 1, b: 2 })` є дійсним, як і `const { a, b } = { a: 1, b: 2 }`.
>
...
- Змінна циклу в циклах [`for...in`](/uk/docs/Web/JavaScript/Reference/Statements/for...in) і [`for...of`](/uk/docs/Web/JavaScript/Reference/Statements/for...of);
...
Іще одним патерном зв'язування об'єкта може бути решта властивостей при деструктуруванні масиву. The inner destructuring destructures from the array created after collecting the rest elements, so you cannot access any properties present on the original iterable in this way.
