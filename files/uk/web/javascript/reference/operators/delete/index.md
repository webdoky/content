...
> [!NOTE]
> Синтаксис мови дозволяє ставити після оператора `delete` широке розмаїття виразів, але лише форми, показані вище, дадуть змістовний результат.
...
Оператор `delete` прибирає з об'єкта задану властивість. У випадку успішного видалення – повертає `true`, інакше – `false`. Всупереч поширеному уявленню (мабуть, пов'язаному з іншими мовами програмування, як то [delete у C++](https://learn.microsoft.com/en-us/cpp/cpp/delete-operator-cpp?view=msvc-170)), оператор `delete` **не має** стосунку до безпосереднього звільнення пам'яті. Керування пам'яттю виконується в непрямий спосіб, шляхом розриву посилань. Дивіться подробиці на сторінці [керування пам'яттю](/uk/docs/Web/JavaScript/Memory_management).
...
> [!NOTE]
> Наступний приклад використовує можливості, доступні лише в нестрогому режимі, але заборонені в строгому, як то неявне створення глобальних змінних та видалення ідентифікаторів.
...
- [Поглиблений аналіз delete](http://perfectionkills.com/understanding-delete/)
