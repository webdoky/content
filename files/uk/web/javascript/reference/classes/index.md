...
Для оголошення полів непотрібні ключові слова типу `let`, `const` і `var`.
...
Наприклад, під час використання методів, що повертають усталений конструктор, таких як {{jsxref("Array.map", "map()")}}, може виникнути потреба повернути батьківський об'єкт `Array` замість `MyArray`. Символ {{jsxref("Symbol.species")}} дозволяє цього досягти таким чином:
