...
Метод **`reduce()`** виконує передану користувачем функцію зворотного виклику на кожному з елементів масиву, передаючи в неї повернене значення від обрахунку попереднього елементу. Кінцевим результатом  обробки всіх елементів масиву функцією `reduce()` є одне значення.

Ймовірно найпростіший для розуміння приклад застосування `reduce()` — це повернення суми всіх елементів масиву.

Функція `reduce()` проходить крізь весь масив, елемент за елементом, з кожним кроком додаючи значення переданого елементу до результату попереднього кроку (цей результат є поточною сумою всіх попередніх кроків), допоки в масиві не залишиться елементів.
...
- `callbackFn`
  - : Функція, яка приймає чотири аргументи:
    ...
    - *array* (масив, який зараз опрацьовується) {{optional_inline}}
...
> У випадку, якщо масив не містить елементів, і *initialValue* не задано, має місце помилка {{jsxref("TypeError")}}.
>
> `reduce` не змінює об'єкт, на якому викликається, проте об'єкт може змінюватись викликом функції *callbackfn*.
>
...
Якщо було передано *initialValue* і масив не прожній, то метод `reduce()` завжди викличе фукнцію зворотного виклику починаючи з індексу 0.

Якщо *initialValue* не передається, то метод `reduce()` буде по різному себе поводити з масивами довжиною більшою за 1, рівною 1 та 0, як показано в наступному прикладі:
...
Якби другим аргументом до `reduce()` було передано *initialValue*, результат виглядав би виглядати так:
...
// де поле "books" — це перелік улюблених книжок
let friends = [{
  name: 'Anna',
...
Застосування {{jsxref("Array.map()")}} після {{jsxref("Array.filter()")}} заставляє програму перебирати масив двічі, але аналогічного результату можна досягти, перебираючи масив лише один раз із {{jsxref("Array.reduce()")}}, таким чином зробивши це більш ефективно. (Якщо вам до вподоби цикли `for`, можна відфільтрувати і перебрати масив за один раз із {{jsxref("Array.forEach()")}}).
 ...
 *
 * @param {array} arr - масив промісів
