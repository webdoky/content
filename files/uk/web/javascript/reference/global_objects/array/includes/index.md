...
- `searchElement`
  ...
  - : Значення для пошуку.
    ...
    > **Зауваження:** `includes()` _враховує регістр_ під час порівняння рядків та окремих букв.

- `fromIndex` {{optional_inline}}

  - : Позиція в масиві, з якої слід почати пошук значення `searchElement`.
...
Якщо дано від'ємний `fromIndex`, обчислений індекс буде обраховано, щоб використати його як позицію елементу в масиві, з якого почнеться пошук `searchElement`. Якщо ж і обчислений індекс менший або дорівнює `0`, буде перевірено весь масив.

```js
// довжина масиву дорівнює 3
// fromIndex дорівнює -100
// обрахований індекс дорівнює 3 + (-100) = -97
...
Наступний приклад ілюструє викликання методу `includes()` на об'єкті [arguments](/uk/docs/Web/JavaScript/Reference/Functions/arguments) функції.
