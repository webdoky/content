...
- `callbackFn`
  ...
  - : Функція для виклику для кожного елемента масиву. Її повернене значення додається у новий масив як один з елементів.
...
Метод `map()` є [копіювальним методом](/uk/docs/Web/JavaScript/Reference/Global_Objects/Array#kopiiuvalni-ta-zminiuvalni-metody). Він не вносить змін до `this`. Проте функція, передана як `callbackFn`, може змінювати масив. Проте зверніть увагу, що довжина масиву зберігається _до_ першого заклику `callbackFn`. Таким чином:
...
- Якщо наявний, іще не оброблений елемент масиву змінюється `callbackFn`, то його значення, передане в `callbackFn`, буде значенням на ту мить, коли такий елемент обробляється. [Видалені](/uk/docs/Web/JavaScript/Reference/Operators/delete) елементи – не обробляються.
