...
Метод `map` викликає передану функцію `callbackFn` **один раз для кожного елемента** масиву, в порядку зростання індексу, і формує новий масив із результатів.
...
Функція `callbackFn` закликається з трьома аргументами: значенням елементу, його порядковим номером і об'єктом масиву, який, власне, перебирається.
...
Метод `map()` є [копіювальним методом](/uk/docs/Web/JavaScript/Reference/Global_Objects/Array#kopiiuvalni-ta-zminiuvalni-metody). Він не змінює `this`.
