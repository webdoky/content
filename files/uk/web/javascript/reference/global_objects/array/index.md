...
Також глибинні копії можна створювати за допомогою метода [`structuredClone()`](/uk/docs/Web/API/structuredClone), причому перевага такого підходу – [переносні об'єкти](/uk/docs/Web/API/Web_Workers_API/Transferable_objects) всередині джерел будуть _перенесені_ в нову копію, а не просто скопійовані.
