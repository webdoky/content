...
3. Метод не запам'ятовує вміст масиву, тож якщо який-небудь індекс змінюється під час ітерування, то може бути оброблено нове значення.
...
5. Методи `reduce` і `reduceRight` мають трохи різні сигнатури і не завжди починають ітерування з першого або останнього елемента.
...
Зміни за ще не обробленими індексами будуть помітні, як тільки ітерування дійде до відповідного індекса:
...
Вставляння _n_ елементів із індексами, більшими за довжину вихідного масиву, не призведе до того, що вони будуть оброблені:
...
Що до методів, які ітерують в порядку спадання індексів, то вставляння призводить до пропускання елементів, а видалення – до їх багаторазової обробки. Спробуйте самі змінити код вище, щоб побачити такі ефекти.
