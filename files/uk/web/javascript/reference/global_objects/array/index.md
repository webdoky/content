...
```js
let last = fruits.pop(); // видаляємо Апельсин (з кінця)
// ["Яблуко", "Банан"]
```
...
```js
let first = fruits.shift(); // видаляємо Яблуко на початку
// ["Банан"]
```
...
```js
let newLength = fruits.unshift('Полуниця'); // додаємо на початку
// ["Полуниця", "Банан"]
```
...
```js
let vegetables = ['Капуста', 'Ріпа', 'Редька', 'Морква'];
...
console.log(vegetables);
// ["Капуста", "Морква"] (початковий масив було змінено)
...
Це — [поверхнева копія](/uk//docs/Glossary/Shallow_copy), виконана за допомогою [оператора розгортання](/uk/docs/Web/JavaScript/Reference/Operators/Spread_syntax). Нижче в розділі [Копіювання масиву](#kopiiuvannia-masyvu) розібрані інші способи скопіювати масив.
...
У прикладі із `3d` рядок `'3d'` _повинен був_ бути в лапках (бо починається з цифри). Але також можливо подавати в лапках і індекси масиву (наприклад, `years['2']` замість `years[2]`), хоча це не є необхідністю.
...
```js
fruits.length = 10;
console.log(fruits); // ['банан', 'яблуко', 'персик', 2 порожнечі, 'манго', 4 порожнечі]
console.log(Object.keys(fruits)); // ['0', '1', '2', '5']
...
Поля та елементи, які будуть повернуті в результаті цього зіставлення, мають наступний вигляд:
...
Всі ці способи дають змогу отримати [_поверхневу копію_](/uk/docs/Glossary/Shallow_copy): елементи верхнього рівня копіюються, проте якщо масив містить вкладені об'єкти чи масиви, то вони й далі будуть посилатись на елементи в початковому масиві.

Якщо необхідна [_глибинна копія_](/uk/docs/Glossary/Deep_copy) всіх елементів — така, в якій навіть вкладені масиви також копіюються, а не просто посилаються на елементи первинного масиву — одним із варіантів буде застосувати {{jsxref("JSON.stringify()")}} для перетворення масиву на рядок JSON, і потім перетворити його назад на масив за допомогою {{jsxref("JSON.parse()")}}.
...
- {{jsxref("Array.from()")}}
  - : Створює новий екземпляр типу `Array` з будь-якого об‘єкта, що подібний до масиву чи надає можливість перебору.
- {{jsxref("Array.isArray()")}}
  - : Повертає `true`, якщо аргумент є масивом, а інакше &mdash; `false`.
- {{jsxref("Array.of()")}}
...
- {{jsxref("Array.prototype.at()")}}{{Experimental_Inline}}
  - : Повертає елемент масиву за заданим індексом. Приймає також від'ємні цілі числа, які розцінюються як номер від останнього елемента, у зворотному порядку.
- {{jsxref("Array.prototype.concat()")}}
  - : Повертає новий масив, утворений шляхом з'єднання даного масиву з іншим масивом чи значенням (або масивами та значеннями).
...
- {{jsxref("Array.prototype.filter()")}}
  - : Повертає новий масив, що містить всі ті елементи поточного, для яких задана аргументом функція повертає `true`.
...
- {{jsxref("Array.prototype.pop()")}}
  - : Видаляє останній елемент масиву та повертає його як результат.
- {{jsxref("Array.prototype.push()")}}
  - : Додає один або більше елементів в кінець масиву, і повертає нове значення властивості `length` масиву.
- {{jsxref("Array.prototype.reduce()")}}
  - : Застосовує функцію до акумулятора і кожного зі значень масиву (зліва направо) з метою зведення його до єдиного значення.
- {{jsxref("Array.prototype.reduceRight()")}}
  - : Застосовує функцію до акумулятора і кожного зі значень масиву (справа наліво) з метою зведення його до єдиного значення.
...
- {{jsxref("Array.prototype.sort()")}}
  - : Сортує елементи масиву _на місці_ та повертає сам масив.
...
- З настанов із JavaScript:
  ...
  - [Індексування властивостей обʼєкту](/uk/docs/Web/JavaScript/Guide/Working_with_Objects#indeksuvannia-vlastyvostei-obiektu)
  - [Індексовані колекції: об'єкт `Array`](/uk/docs/Web/JavaScript/Guide/Indexed_collections#obiekt-array)
