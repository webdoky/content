...
Методи `{{jsxref("Promise.prototype.then()")}}`, `{{jsxref("Promise.prototype.catch()")}}` і `{{jsxref("Promise.prototype.finally()")}}` використовуються, щоб приєднати якусь наступну дію до промісу, який залагоджується.
