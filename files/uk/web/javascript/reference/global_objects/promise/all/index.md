...
`Promise.all` очікує на всі сповнення (або до першого відхилення).
...
// застосування setTimeout дає змогу виконати код після очищення стека
setTimeout(function() {
    console.log(p);
    console.log(p2);
    console.log(p3);
...
### Асинхронність синхронності методу `Promise.all`
...
### Поведінка "fail-fast" методу `Promise.all`
