...
Якщо передані аргументи не є рядками, їх буде зведено до рядкового типу перед безпосередньою конкатенацією.

## Швидкодія

Категорично рекомендується вживати {{jsxref("Operators/Assignment_Operators", "оператори присвоєння", "", 1)}} (`+`, `+=`) замість методу `concat()`.
