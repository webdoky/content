...
- `substr`
  - : {{jsxref("String", "Рядок")}}, який буде замінений переданим `newSubstr`. Він сприймається як рядковий літерал, і _не_ інтерпретується як регулярний вираз. Заміниться лише перший збіг зі шуканим рядком.
- `newSubstr` (заміна)
...
| `$n`      | Де `n` — додатне ціле число, менше за 100. Вставляє збіг з відповідною групою в дужках, за умови, якщо першим аргументом було передано об'єкт {{jsxref("RegExp", "регулярного виразу")}}. Важлива деталь - нумерація цих груп починається з `1`. Як вказана група під номером `n` відсутня (скажімо, якщо номер групи — 3), вона буде вставлена буквально (в цьому випадку як рядок `$3`).                     |
...
Другим аргументом можна також передати функцію. В цьому випадку функція викликається одразу, як збіг знайдено. Результат виконання функції (повернене значення) буде використано як рядок заміни. (**Зауваження:** Згадані вище спеціальні патерни для заміни в цьому випадку _не_ застосовуються.)
...
> **Примітка:** Зверніться до [цих настанов](/en-US/docs/Web/JavaScript/Guide/Regular_Expressions) за повнішими
> роз'ясненнями щодо регулярних виразів.

### Застосування прапорців "глобальна заміна" та "знехтувати регістром" з replace()

Глобальна заміна може виконуватись лише із застосуванням регулярного виразу. В наступному прикладі регулярний вираз містить [прапорці "глобальний пошук" та "знехтувати регістром"](/en-US/docs/Web/JavaScript/Guide/Regular_Expressions#advanced_searching_with_flags_2), що дозволяє функції `replace()` замінити всі входження `'яблука'` на рядок з `'апельсини'`.
...
Наступний скрипт міняє слова місцями в рядку. Для отримання тексту заміни, скрипт використовує [групи з захопленням](/en-US/docs/Web/JavaScript/Guide/Regular_Expressions/Groups_and_Ranges) і патерни заміни `$1` та `$2`.
...
В цьому прикладі, всі наявні в рядку великі літери перетворюються в літери нижнього регістру, а перед місцем збігу вставляється дефіс. Важлива деталь — перед тим, як повернути елемент як рядок заміни, над ним слід провести додаткові операції.
...
Оскільки в цьому випадку нам потрібно додатково змінити _знайдений збіг_ перед остаточною заміною, ми повинні вжити функцію. Це змушує програму спершу знайти зміг, до виконання методу {{jsxref("String.prototype.toLowerCase()", "toLowerCase()")}}. Якби ми спробували зробити заміну без функції, метод {{jsxref("String.prototype.toLowerCase()", "toLowerCase()")}} не мав би ефекту.
...
### Заміна градусів Фаренгейтах їхнім еквівалентом по шкалі Цельсія
