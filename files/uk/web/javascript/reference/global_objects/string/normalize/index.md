...
Кожному окремому символу Unicode призначає числове значення, назване _кодовою точкою_. Наприклад, для літери `"A"` визначено кодову точку U+0041. Однак, інколи один і той самий абстрактний символ можна позначати декількома різними кодовими точками, або навіть їх послідовністю. Наприклад, символ `"ñ"` можна позначити одним із двох способів:
...
Варто зауважити, що довжина форми, нормалізованої методом `"NFD"`, дорівнює `2`. Це відбувається тому, що `"NFD"` видає **декомпоновану** версію канонічної форми, в якій поодинокі кодові точки розділені на декілька базових. Декомпонована канонічна форма літери `"ñ"` — це послідовність `"\u006E\u0303"`.
...
Як і з канонічною нормалізацією, можна запросити компоновану чи декомпоновану сумісні форми шляхом передачі аргументів `"NFKD"` чи `"NFKC"`, відповідно.
