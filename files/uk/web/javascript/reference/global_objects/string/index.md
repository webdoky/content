...
Рядкові примітиви та об'єкти-рядки поділяють значну частину своєї поведінки, але мають певні важливі відмінності та каверзи. Дивіться розділ "[Рядки-примітиви та рядки-об'єкти](#riadky-prymityvy-ta-obiekty-string)" нижче.
...
### Звертання до окремих символів

Є два способи звернутися до окремого символу рядка. Перший — це метод {{jsxref("String/charAt", "charAt()")}}:

```js
"cat".charAt(1); // дає значення "a"
```

Інший спосіб – розглянути рядок як масивоподібний об'єкт, де кожний символ відповідає певному числовому індексу:
...
### Рядки-примітиви та об'єкти String

Зауважте, що JavaScript розрізняє об'єкти `String` та {{Glossary("Primitive", "примітивні рядкові")}} значення. (Це стосується також {{jsxref("Boolean", "булевих значень")}} і {{jsxref("Number", "чисел")}}.)
...
> [!WARNING]
> Використання `String` як конструктора повинно траплятися рідко.
...
Об'єкт `String` можна завжди перетворити на його примітивний відповідник, скориставшись методом {{jsxref("String/valueOf", "valueOf()")}}.
...
- {{jsxref("String/String", "String()")}}
  - : Створює об'єкти `String`. Коли викликається як функція, то повертає примітивні значення типу String.
...
- {{jsxref("String.prototype.indexOf()")}} (індекс (підрядка))
  - : Повертає індекс першого у поточному рядку входження `searchValue`, або `-1`, якщо жодного не знайдено.
- {{jsxref("String.prototype.isWellFormed()")}} (чи є добре сформованим)
  - : Повертає булеве значення, яке вказує, чи містить поточний рядок [самотні сурогати](#symvoly-utf-16-kodovi-tochky-unicode-ta-hrafemni-klastery).
- {{jsxref("String.prototype.lastIndexOf()")}} (останній індекс (підрядка))
  - : Повертає індекс останнього у поточному рядку входження `searchValue`, або `-1`, якщо жодного не знайдено.
...
- {{jsxref("String.prototype.padEnd()")}} (заповнити кінець)
  - : Заповнює поточний рядок з кінця переданим рядком, а тоді повертає новий рядок довжиною `targetLength`.
- {{jsxref("String.prototype.padStart()")}} (заповнити початок)
  - : Заповнює поточний рядок з початку переданим рядком, а тоді повертає новий рядок довжиною `targetLength`.
...
- [`String.prototype[Symbol.iterator]()`](/uk/docs/Web/JavaScript/Reference/Global_Objects/String/Symbol.iterator)
  - : Повертає новий об'єкт-ітератор, який перебирає всі кодові точки значення String, повертаючи кожну з них як окреме значення String.

### Методи для обгортання в HTML

> [!WARNING]
> Нерекомендовані. Уникайте вживання цих методів.
>
> Вони мають обмежене застосування, оскільки засновані на дуже старому стандарті HTML і надають лише підмножину наразі доступних тегів та атрибутів HTML. Чимало з них нині створює нерекомендовану чи нестандартну розмітку. На додачу до цього, ці методи виконують склеювання рядків, без жодних валідації й очищення, що робить їх потенційною загрозою безпеці, коли безпосередньо вставляти їх результат за допомогою [`innerHTML`](/uk/docs/Web/API/Element/innerHTML). Натомість слід використовувати [DOM API](/uk/docs/Web/API/Document_Object_Model), як то [`document.createElement()`](/uk/docs/Web/API/Document/createElement).
