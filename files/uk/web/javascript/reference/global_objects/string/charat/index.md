...
- `index`
  - : Ціле число в діапазоні від `0` і до `str.length - 1`. Якщо `index` не можливо перетворити на число, або його просто не задано — замість нього буде використано усталене значення `0`, і таким чином повернено перший символ рядка `str`.
...
Наступний код дає змогу пересвідчитись, що під час проходження по рядку цикл завжди віддасть цілий символ, навіть якщо рядок містить символи, що не належать базовій багатомовній площині.
...
function getWholeChar(str, i) {
  ...
  // Старший сурогатний символ (останнє шістнадцятичне значення можна
  // змінити на 0xDB7F, щоб повернути старший сурогат як окремий символ)
  ...
  // (тут можна замінити останнє шістнадцятичне значення на 0xDB7F, щоб повернути
  // старший сурогат окремим символом приватної площини unicode)
...
for (let i = 0, chr; i < str.length; i++) {
  ...
  // новим значенням 'i' (яке змінюється лише під часо обробки сурогатної пари)
...
### Виправлення методу charAt() так, щоб він підтримував не лише символи базової багатомовної площини (BMP)
