...
- Значення {{jsxref("NaN")}} вважається таким самим як `NaN` (хоча `NaN !== NaN`), а всі інші значення вважаються однаковими у відповідності з семантикою оператора `===`.
- В діючій специфікації ECMAScript, `-0` та `+0` вважаються рівними, хоча в більш ранніх чорновиках це було не так. Зверніться до _"Value equality for -0 and
  0"_ в таблиці [Сумісності з браузерами](#browser_compatibility) за подробицями.

### Порівняння `Object` та `Map`

{{jsxref("Object")}} дуже схожий до `Map` — обидва дозволяють задавати значеня ключам, отримувати ті значення, видаляти ключі і визначати, чи зберігається щось за певним ключем. З цих міркувань (а ще тому, що не існувало вбудованих альтернатив), історично `Object` використовується як `Map`.
...
<table class="standard-table">
  ...
  <tbody>
    <tr>
      ...
      <td>
        <code>Map</code> усталено не містить ніяких ключів. Він містить лише те, що було явно покладено в нього.
      </td>
      <td>
        <p>
        ...
        <div class="notecard note">
          <p>
            <strong>Зауваження:</strong> Від ES5, цю проблему можна обійти за допомогою {{jsxref("Object.create", "Object.create(null)")}}, хоча це рідко застосовується.
          </p>
        </div>
      </td>
    </tr>
    <tr>
      <th scope="row">Тип ключів</th>
      <td>
        Ключі в <code>Map</code> можуть мати будь-яке значення (включно з функціями, об'єктами, чи будь-яким примітивним значенням).
      </td>
      <td>
    ...
    <tr>
      ...
      <td>
        ...
        <p>
          ...
          {{jsxref("Object.getOwnPropertySymbols")}} робить те саме, але лише для властивостей з ключами-сомволами, і т. д.)
        </p>
      </td>
    </tr>
    <tr>
      ...
      <td>
        Кількість елементів всередині <code>Object</code> слід обраховувати вручну.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>
        ...
        <p>
          Нативна підтримка для парсингу JSON у {{jsxref("Object")}} використовуючи {{jsxref("JSON.parse()")}}.
        </p>
      </td>
    </tr>
  </tbody>
...
- {{jsxref("Map.@@species", "get Map[@@species]")}}
  - : Функція конструктора, який застосовується для створення похідних об'єктів.
...
// Зливаємо докупи два об'єкти `Map`. Останній повторений ключ перемагає.
...
// Зливаємо докупи об'єкти `Map` з масивом. Останній повторений ключ перемагає.
