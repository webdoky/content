...
В основі своїй дата в JavaScript задається як кількість мілісекунд, що минули з опівночі 1 січня 1970, UTC. Ці час та дата — не одне й те ж, що **епоха UNIX** (кількість секунд, що минули з опівночі 1 січня 1970, UTC), яка переважно використовується як основне значення для записаних комп'ютером часу та дати.
...
Існує кілька різних способів як отримати дату в різних форматах, так і виконати перетворення між часовими зонами. Особливо корисні ті функції, які дають результат у форматі Всесвітньому координованому часі (UTC) — загальному форматі часу, визначеному у світовому стандарті часу (Цей час також історично відомий як _Середній час за Гринвічем (GMT)_, оскільки UTC лежить вздовж меридіану, який включає Лондон (і розташований неподалік Гринвіч) у Великобританії). Користувацький пристрій надає локальний час.

На додачу до методів для читання і зміни окремих складових локальної дати й часу (таких як {{jsxref("Date.getDay", "getDay()")}} і {{jsxref("Date.setHours", "setHours()")}}), існують також версії тих самих методів, що читають та маніпулюють датою в UTC (такі як {{jsxref("Date.getUTCDay()", "getUTCDay()")}} і {{jsxref("Date.setUTCHours", "setUTCHours()")}}).
...
- {{jsxref("Date.parse()")}}
  ...
  - : Розбирає рядок з датою і повертає відповідну кількість мілісекунд від 1 січня 1970 року 00:00:00 UTC, без врахування високосних секунд.
    ...
    > **Зауваження:** Дуже не рекомендується розбирати рядки з датами за допомогою `Date.parse` через неоднорідність браузерів.

- {{jsxref("Date.UTC()")}}
  - : Приймає ті ж аргументи, що й найдовша форма конструктора (тобто від 2 до 7) і повертає кількість мілісекунд від 1 січня 1970 року, 00:00:00 UTC, без врахування високосних секунд.
...
- {{jsxref("Date.prototype.getHours()")}}
  - : Повертає годину (`0`–`23`) з вказаної дати, відповідно до локального часу.
...
- {{jsxref("Date.prototype.getTime()")}}
  - : Повертає числове значення вказаної дати як число мілісекунд, що минули від 1 січня 1970 року 00:00:00 UTC. (Для більш ранньої дати буде повернено негативне значення.)
...
- {{jsxref("Date.prototype.setTime()")}}
  - : Встановлює значення об'єкту {{jsxref("Date")}} відповідно до часу, переданого як число мілісекунд, що сплили з 1 січня 1970 року 00:00:00 UTC. Для задання більш ранніх дат використовуються негативні значення.
...
- {{jsxref("Date.prototype.toGMTString()")}}
  - : Повертає рядок, що відповідає даті в {{jsxref("Date")}} відносно часової зони GMT (UTC). Застосовуйте {{jsxref("Date.prototype.toUTCString()", "toUTCString()")}} натомість.
...
> **Зауваження:** Категорично не рекомендується розбирати властивості дати за допомогою конструктора `Date` (і `Date.parse`, вони еквівалентні), у зв'язку з неоднорідностями в реалізації браузерів.
...
Вираження величини проміжку часу в одиницях, більших за години, хвилини чи секунди, потребує вирішення певної кількості проблем, зокрема різна довжина днів (пов'язана з переходом між літнім та зимовим часом), місяців, і років. Рекомендується ретельно вивчити ці неоднорідності _до_ початку роботи з великими проміжками часу.
