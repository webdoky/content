...
Об'єкти **`Date`** в JavaScript представляють єдиний момент часу в платформонезалежному форматі. Об'єкти типу `Date` містять число, яке відповідає кількості мілісекунд, що минули з 1 січня 1970 року за UTC (Всесвітнім координованим часом).

> **Зауваження:** TC39 наразі працює над [Temporal](https://tc39.es/proposal-temporal/docs/index.html), новим API для дати й часу. Більше про це [в блозі Igalia (англ.)](https://blogs.igalia.com/compilers/2020/06/23/dates-and-times-in-javascript/). Однак цей інтерфейс іще не готовий для застосування в програмних продуктах.
...
> **Зауваження:** Важливо мати на увазі, що, хоча значення часу в центрі об'єкту дати задане за UTC, базові методи для отримання дати та часу (чи їх складових) працюють в локальних часовій зоні та поясі (тобто беруться системні значення).
...
> **Зауваження:** В браузерах, які мають підтримку {{domxref("performance_property", "Web Performance API", "", 1)}} — вимірів часу високої точності — метод {{domxref("Performance.now()")}} надасть значно точніші й надійніші виміри часу, аніж {{jsxref("Date.now()")}}.
