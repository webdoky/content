...
Метод **`JSON.stringify()`** перетворює JavaScript-об'єкт або значення в рядок JSON, додатково замінюючи значення, якщо була вказана необов'язкова функція для заміни, або ж додатково вибираючи лише вказані властивості, якщо було передано масив для заміни.
...
- `value`
  - : Значення, яке буде перетворене на рядок JSON.
...
- `space` {{optional_inline}}
  ...
  - : {{JSxRef("String", "Рядок")}} або {{JSxRef("Number", "Число")}}, який буде використано для вставки пробільних символів у отриманий рядок JSON з міркувань розбірливості.

    Якщо передане значення має тип `Number`, воно означатиме кількість пробільних символів, використаних для створення відступів. Ця кількість обмежена 10-ма (якщо передане число більше за 10, кількість пробілів все одно буде `10`). Значення, які менші за 1, позначають, що жодного пробільного символу вжито не буде.
...
Метод `JSON.stringify()` перетворює значення аргументу у рядкове значення, записане JSON-нотацією:

- Якщо значення має метод [`toJSON()`](#tojson_behavior), він відповідатиме за те, які саме дані буде серіалізовано.
- Об'єкти {{JSxRef("Boolean")}}, {{JSxRef("Number")}} та {{JSxRef("String")}} під час серіалізації перетворюються у їхні відповідні примітивні значення, згідно з традиційною семантикою таких перетворень.
- {{JSxRef("undefined")}}, {{JSxRef("Function", "функції")}}, та {{JSxRef("Symbol", "Символи")}} не є валідними значеннями JSON. Якщо якесь із цих значень трапляється під час перетворення, воно або опускається (в об'єктах), або ж змінюється на {{JSxRef("null")}} (якщо трапляється в масиві). `JSON.stringify()` може повернути `undefined`, якщо передати в нього їхні "чисті" значення, як от `JSON.stringify(function(){})` або `JSON.stringify(undefined)`.
...
**Як функція**, він приймає два аргументи: _ключ_ та _значення_, що перетворюються. Об'єкт, на якому було знайдено цей ключ, передається у функцію з параметром `this`.
...
- Якщо повертається {{JSxRef("Function")}}, {{JSxRef("Symbol")}}, або {{JSxRef("undefined")}}, ця властивість _не буде_ включена у виводі.
- Якщо ж повертається будь-який інший об'єкт, його буде рекурсивно серіалізовано, з викликом функції `replacer` на кожній з його властивостей.
...
Якщо в `replacer` передано масив, то елементи масиву будуть позначати імена властивостей об'єкта, які слід залишити в отриманому рядку JSON.
...
// Перетворення об'єкту у JSON-рядок із JSON.stringify(),
...
Рушії, що реалізовують [специфікацію JSON.stringify з правильним формуванням](https://github.com/tc39/proposal-well-formed-stringify) опрацьовують поодинокі сурогати — будь-які кодові одиниці від U+D800 до U+DFFF — із застосуванням керівних послідовностей Unicode, замість вживання їх буквально. До цієї зміни `JSON.stringify` міг повертати поодинокі сурогати в разі, якщо такі сурогати знаходилися в початковому рядку. Такі рядки потім неможливо було коректно перекодувати в UTF-8 чи UTF-16:

```js
JSON.stringify("\uD800"); // '"�"'
```

Проте з цією зміною `JSON.stringify` відображає поодинокі сурогати за допомогою екранованих послідовностей JSON, які _можна_ коректно перекодувати в рядок UTF-8 чи UTF-16:

```js
JSON.stringify("\uD800"); // '"\\ud800"'
```

Ця зміна має бути зворотно сумісна доти, доки результат `JSON.stringify` передається до такого API, як `JSON.parse`, який прийматиме будь-який валідний JSON-текст, оскільки вони будуть вважати юнікодні екрановані поодинокі сурогати рівними самим сурогатам. _Лише_ під час прямої інтерпретації результатів виконання `JSON.stringify` слід уважно опрацьовувати два можливих варіанти кодування таких кодових одиниць.
