...
Несуттєві {{Glossary("whitespace", "пробіли")}} можуть зустрічатися всюди, крім всередині `JSONNumber` (числа не вміщають пробілів) і `JSONString` (там вони тлумачаться як відповідні символи в рядку, або ж викликають помилку). Символ табуляції (U+0009), повернення каретки (U+000D), переведення рядка (U+000A) і пробіл (U+0020) – єдині допустимі пробільні символи.
