...
// АБО
const re = new RegExp(/ab+c/, 'i'); // конструктор з літералом регулярного виразу як першим аргументом
```
...
Слід передавати першим аргументом в конструктор `RegExp()` рядок, коли є потреба [збудувати регулярний вираз на основі динамічного введення](#pobudova-rehuliarnoho-vyrazu-na-osnovi-dynamichnoho-vvedennia).
...
Так було вирішено через те, що `@@match` є найбільш показовою властивістю щодо того, що щось створено для пошуку збігів. (`exec` також могла бути використана, але через те, що це не символьна властивість, було б забагато помилкових спрацювань.) Серед місць, що обробляють регулярні вирази по-особливому:
...
Починаючи з Firefox 34, у випадку захоплення групи з квантифікаторами, що не дають їй спрацювати, текст збігу групи захоплення – `undefined`, а не порожній рядок:
...
- Розділ [Настанов JavaScript](/uk/docs/Web/JavaScript/Guide) – [Регулярні вирази](/uk/docs/Web/JavaScript/Guide/Regular_Expressions)
