...
Оголошення **`let`** оголошує локальну змінну з блоковою областю видимістю, необов'язково ініціалізуючи її значенням.
...
Змінна в TDZ іще не була ініціалізована значенням, і будь-які спроби звернутися до неї призведуть до {{jsxref("ReferenceError")}}.
...
Проте поєднання оголошень **`var`** і **`let`** нижче – {{jsxref("SyntaxError")}}, у зв'язку з тим, що **`var`** піднімається нагору блоку.
