...
switch (foo) {
  case -1:
    ...
    break;
  case 0: // foo дорівнює 0, а отже умова відповідає, тож цей блок буде виконано
    console.log(0);
...
Цей метод заснований на тому факті, що за відсутності `break` під пунктом `case` виконання продовжиться наступним пунктом `case` незалежно від того, чи відповідає він критерію. (Детальніше в розділі [Що трапиться, якщо забути `break`](#what_happens_if_i_forgot_a_break))
