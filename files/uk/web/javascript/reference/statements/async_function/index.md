...
Асинхронна функція - це функція, що оголошена з ключовим словом `async`, і всередині якої дозволене вживання ключового слова `await`. Ключові слова `async` та `await` дають змогу записувати асинхронну (засновану на промісах) поведінку в чистішому стилі, уникаючи необхідности явно задавати ланцюжки промісів.
...
Код після кожного виразу `await` можна сприймати як наче він існує всередині функції зворотного виклику `.then`. Таким чином, ланцюжок промісів поступово формується з кожним повторним входом у функцію. Повернене значення стає кінцевою ланкою ланцюжка.

В наступному прикладі очікування на виконання двох промісів. Процес виконання функції `foo` відбувається у три етапи.
...
Під час послідовного старту у функції `sequentialStart`, виконання зупиняється на 2 секунди на першому `await`, і потім іще на секунду на другому `await`. Другий таймер не створюється, поки не сплине час першого, тому загалом код завершує роботу за 3 секунди.

Під час одночасного старту у функції `concurrentStart`, обидва таймери створюються, і потім очікуються виразами `await`. Таймери виконують відлік одночасно, тобто код завершить роботу радше за 2, ніж за 3 секунди. Інакше кажучи — тоді, коли завершить відлік найповільніший таймер. Втім, виклики `await` все ще виконуються послідовно, тобто другий `await` чекатиме на завершення першого. В цьому випадку результат швидшого таймера обробляється після завершення роботи повільнішого.

Якщо потрібно безпечним чином виконати паралельно два чи більше завдання, слід очікувати в коді на виклик [`Promise.all`](/uk/docs/Web/JavaScript/Reference/Global_Objects/Promise/all) чи [`Promise.allSettled`](/uk/docs/Web/JavaScript/Reference/Global_Objects/Promise/allSettled).
...
Цей можна переписати як одну асинхронну функцію, як це показано нижче:
...
- ["Декорування асинхронних функцій у Javascript" на "innolitics.com" (англ.)](https://innolitics.com/10x/javascript-decorators-for-promise-returning-functions/)
