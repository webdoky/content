...
- `exceptionVar` {{optional_inline}}
  - : Необов'язковий [ідентифікатор або патерн](#zminna-vyniatku), що містить перехоплений виняток для відповідного блока `catch`. Якщо блок `catch` не використовує значення винятку, то можна опустити `exceptionVar` і дужки біля нього.
- `finallyStatements`
...
Коли в блоку `try` викидається виняток, то `exceptionVar` (тобто `e` в `catch (e)`) містить значення цього винятку. Цю змінну можна використовувати для отримання інформації про викинутий виняток. Вона доступна лише в {{Glossary("Scope", "області видимості")}} блоку `catch`.
...
Змінні, зв'язані з положенням `catch`, живуть в тій же області видимості, що й сам блок `catch`, тож всі змінні, оголошені в блоці `catch`, не можуть мати назв, що збігаються з назвами змінних, зв'язаних з положенням `catch`. (Існує [один виняток з цього правила](/uk/docs/Web/JavaScript/Reference/Deprecated_and_obsolete_features#instruktsii), але такий запис – нерекомендований.)
...
Змінна винятку – доступна для запису. Наприклад, можна нормалізувати значення винятку, аби мати певність, що це об'єкт {{jsxref("Error")}}.
