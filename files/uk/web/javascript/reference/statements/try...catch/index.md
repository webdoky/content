...
- `exception_var` {{optional_inline}}
  - : Необов‘язковий ідентифікатор, що містить перехоплений виняток для відповідного блока `catch`. Якщо блок `catch` не використовує значення винятку, то можна опустити `exception_var` і дужки біля нього: `catch {...}`.
- `finally_statements`
...
Коли в блоку `try` викидається виняток, _`exception_var`_ (тобто `e` в `catch (e)`) зберігає значення винятку. Цей ідентифікатор можна використати для отримання інформації про викинутий виняток. Він доступний лише в {{Glossary("Scope", "області видимості ")}} блоку `catch`. Якщо значення винятку непотрібне, цей ідентифікатор можна опустити.
