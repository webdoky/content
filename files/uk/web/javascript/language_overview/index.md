...
- [BigInt](/uk/docs/Web/JavaScript/Data_structures#typ-bigint): використовується для великих цілих чисел довільної ширини.
...
Тип [BigInt](/uk/docs/Web/JavaScript/Reference/Global_Objects/BigInt) є цілим числом довільної ширини. Його поведінка подібна до поведінки цілих чисел мови C (наприклад, ділення обрізається до нуля), окрім того, що таке число може зростати нескінченно. BigInt задається у вигляді числового літерала з суфіксом `n`.ß
...
Рядки мають [службові методи](/uk/docs/Web/JavaScript/Reference/Global_Objects/String#metody-prymirnyka) для оперування рядком і звертання до інформації про нього. Через те, що всі примітиви задумані незмінними, такі методи повертають нові рядки.
...
Змінній, оголошеній за допомогою `const`, не може повторно бути присвоєно значення.
...
Об'єкти завжди є посиланнями, тож якщо не скопіювати об'єкт явно, то видозміни об'єкта будуть помітні зовнішньому світові.
...
Доступна низка інших синтаксисів параметрів. Наприклад, [синтаксис решти параметрів](/uk/docs/Web/JavaScript/Reference/Functions/rest_parameters) дає змогу зібрати всі надлишкові параметри, передані викликачем, у масив, подібно до `*args` у Python. (Оскільки JS не має на рівні мови іменованих параметрів, то `**kwargs` немає.)
...
Параметр решти збереже всі аргументи _після_ місця, де оголошений, але не до нього. наприклад, `function avg(firstValue, ...args)` збереже перше значення, передане до функції, у змінній `firstValue`, а решту аргументів – у `args`.
...
Було згадано, що JavaScript не має іменованих параметрів. Проте їй можливо реалізувати за допомогою [деструктурування об'єктів](/uk/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment), котре дає змогу зручно запаковувати й розпаковувати об'єкти.
...
Анонімні функції можуть бути корисними в іще один спосіб: їх можна і оголосити, і зразу закликати – в межах одного виразу, що зветься [Негайно закликаним функційним виразом (IIFE)](/uk/docs/Glossary/IIFE):
...
JavaScript дозволяє викликати функції рекурсивно. Це особливо корисно при обробці деревних структур, як от структури DOM у браузері.
...
Також це чудова протидія спокусі глобальних змінних. При написанні складного коду нерідко здається спокусливою ідея використати глобальні змінні для спільних між декількома функціями значень, що призводить до коду, котрий важко підтримувати. Вкладені функції можуть мати спільні функції у батьківській функції, тож цей механізм можна використовувати для сполучення функцій без забруднення глобальної області видимості.
...
Також JavaScript описує модульну систему, котра підтримується більшістю середовищ виконання. Модуль – це зазвичай файл, котрий ідентифікується шляхом або URL до себе. Інструкції [`import`](/uk/docs/Web/JavaScript/Reference/Statements/import) і [`export`](/uk/docs/Web/JavaScript/Reference/Statements/export) можна використовувати для обміну даних між модулями:

```js
import { foo } from "./foo.js";

// Неекспортовані змінні є внутрішніми щодо модуля
const b = 2;

export const a = 1;
```

На відміну від Haskell, Python, Java тощо, вирішення модулів JavaScript цілком визначається середовищем: зазвичай воно засноване на URL або шляхах до файлів, тож відносні шляхи до файлів "просто працюють" і є відносними щодо шляху поточного модуля, а не якогось кореневого шляху проєкту.
