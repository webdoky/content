...
У JavaScript, об'єкт — це окрема сутність, зі властивостями й типом. Її можна порівняти зі, наприклад, чашкою. Чашка — це об'єкт, із властивостями. Чашка має колів, дизайн, масу, матеріал, з якого її зроблено тощо. Так само й об'єкти JavaScript можуть мати властивості, що визначають їхні характеристики.
...
Наведений вище приклад можна також переписати за допомогою **[ініціалізатора об'єкта](#object_initializers)**, який є обгорнутим у фігурні дужки (`{}`) списком зі нуля чи більше розділених комами пар із назви властивості й відповідного їй значення:
...
Не присвоєні властивості об'єкта є {{jsxref("undefined", "невизначеними")}} (а не [`null` (пустими)](/uk/docs/Web/JavaScript/Reference/Operators/null)).
...
// зверніть увагу, послідовність властивостей у переліку, виведеному в консоль, не збігається зі послідовністю, вякому їх було створено.
...
Можна застосовувати запис через квадратні дужки, із [`for...in`](/uk/docs/Web/JavaScript/Reference/Statements/for...in) для перебирання всіх перелічуваних властивостей об'єкта. Для ілюстрації того, як це працює, нижче наведено функцію, яка показує властивості об'єкта, якщо до неї передати в аргументах сам об'єкт, та його назву:
...
Існує три нативних способи перебрати чи отримати список властивостей об'єкта:
...
Застосування ініціалізаторів об'єктів також іноді називають створенням об'єктів за допомогою літерала. Термін "об'єктний ініціалізатор" сумісний з термінологію, прийнятою в середовищі C++.
...
…де `obj` — це назва нового об'єкта, кожна назва властивості до двокрапки — ідентифікатор (або ім'я, або число, або рядковий літерал), та кожний `valueN` — вираз, чиє значення присвоюється назві властивості. Назва `obj` та присвоєння не є обов'язковими — немає сенсу створювати змінну, якщо немає потреби звертатися до цього об'єкта десь деінде. (Зауважте, що може виникнути потреба обгорнути об'єктний літерал в дужки, якщо об'єкт знаходиться в місці, де очікується вираз, аби компілятор не сприймав літерал за блокову інструкцію.)
...
Також можна створювати масиви за допомогою об'єктних ініціалізаторів. Більше про це в розділі [літерали масивів](/uk/docs/Web/JavaScript/Guide/Grammar_and_types#array_literals).

### Застосування функції-конструктора

Окрім цього, можна створювати об'єкти за допомогою таких двох кроків:

1. Означити тип об'єкта шляхом написання функції-конструктора. Існує поширена конвенція називати конструктори з великої літери, з гарним обґрунтуванням.
...
Слід зазначити, що завжди можна додати нову властивість до створеного раніше об'єкта. Наприклад, така інструкція
...
Також можна створювати об'єкти шляхом застосування методу {{jsxref("Object.create()")}}. Цей метод може бути дуже корисним, адже він дає можливість вказати прототип для об'єкта, який створюється, без необхідності в оголошенні окремої функції-конструктора.
...
Також можна означити метод для всього об'єктного типу, шляхом додання такого означення у конструкторі об'єкта. Наприклад, можна додати визначення функції, яка б форматувала і показувала властивості означених раніше об'єктів `Car`:
...
Цю функцію можна зробити методом об'єкта `Car`, шляхом додавання такої інструкції до означення об'єкта:
...
Тут `this` посилається на об'єкт, в контексті якого його вжито. Можна створити нову функцію під назвою `howOldAmI()`, що друкує речення, яке каже, скільки персоні років.
...
- або означені всередині [об'єктних ініціалізаторів](#object_initializers),
- або будь-коли пізніше додані до будь-якого об'єкта за допомогою метода для додавання гетера чи сетера.

Все, що потрібно зробити, аби додати гетер чи сетер через [об'єктний ініціалізатор](#object_initializers) — це вказати перед методом гетера префікс `get`, а перед сетером, відповідно, — `set`. Звісно, метод гетера не повинен приймати вхідних параметрів, тоді як метод сетера повинен очікувати на один і лише один параметр (нове значення для встановлення). Приміром:
...
myObj.c = 50;         //   <-- А тут діє метод сетера c(x) method
...
Можна видалити не успадковану властивість, шляхом використання оператора [`delete`](/uk/docs/Web/JavaScript/Reference/Operators/delete). Наступний код ілюструє, як видалити властивість:
...
Більше інформації про оператори порівняння можна знайти у розділі [оператори порівняння](/uk/docs/Web/JavaScript/Reference/Operators#equality_operators).
