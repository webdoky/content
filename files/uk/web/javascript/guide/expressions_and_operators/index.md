...
Ці оператори об'єднують операнди, або сформовані операторами вищого пріоритету, або одним із [базових виразів](#bazovi-vyrazy). Повний і розгорнутий список операторів та виразів доступний також у [довіднику](/uk/docs/Web/JavaScript/Reference/Operators).
...
Оператор присвоєння присвоює значення своєму лівому операндові на основі значення правого операнда. Простий оператор присвоєння – це знак дорівнює(`=`), він присвоює значення свого правого операнда своєму лівому операндові.
...
Якщо вираз обчислюється до [об'єкта](/uk/docs/Web/JavaScript/Guide/Working_with_objects), then the left-hand side of an assignment expression may make assignments to properties of that expression.
For example:
...
Загалом, присвоєння вживаються в межах оголошень змінних (тобто вкупі з [`const`][], [`let`][] або [`var`][]), або ж як окремі інструкції).
...
1. Починається обчислення виразу `x[f()] = g()`.
   ...
   4. Тепер `3` присвоюється `x[2]`. (Цей крок має успіх лише за умови того, що `x` присвоєний [об'єкт](/uk/docs/Web/JavaScript/Guide/Working_with_objects).)
...
<table class="fullwidth-table">
  ...
  <tbody>
    ...
    <tr>
      <td>
        <a href="/uk/docs/Web/JavaScript/Reference/Operators/Unary_negation">Унарної протилежності</a> (<code>-</code>)
      </td>
...
- `false && щонебудь` – із закороченням обчислюється в false.
- `true || щонебудь` – із закороченням обчислюється в true.

Правила логіки гарантують, що такі обчислення завжди є коректними. Зверніть увагу на те, що частина _щонебудь_ виразів вище – не обчислюється, тож жодні побічні ефекти цих частин – не подіють.
