...
На додачі до нововизначеної змінної, як це показано вище, масив може бути присвоєний властивості нового або наявного об'єкта:
...
> **Примітка:** Якщо передати оператору масиву значення, що не є цілим числом, у код вище, то в об'єкті, що представляє масив, буде створена властивість, а не елемент масиву.
>
> ```js
> const arr = [];
> arr[3.4] = "Помаранчі";
...
Слід пам'ятати, що індекси Array JavaScript починаються від `0`, а не `1`. А отже, властивість `length` буде на одиницю більшою за найбільший індекс, присутній в масиві:
...
Такий підхід дає змогу уникнути накладних витрат з перевірки довжини масиву й пересвідчитися, що змінна `div` щоразу отримує поточний елемент, із покращеною зручністю.
...
- якщо `a` менше за `b`, згідно з системою сортування, то повернути `-1` (або будь-яке від'ємне число)
- якщо `a` більше за `b`, згідно з системою сортування, то повернути `1` (або будь-яке додатне число)
- якщо `a` і `b` вважаються рівносильними, то повернути `0`.
...
[Типізовані масиви JavaScript](/uk/docs/Web/JavaScript/Typed_arrays) є масивоподібними об'єктами й пропонують механізм для звертання до необроблених двійкових даних. Як ви вже знаєте, об'єкти {{jsxref("Array")}} динамічно збільшуються та зменшуються і можуть зберігати будь-яке значення JavaScript. Рушії JavaScript виконують оптимізації, завдяки яким такі масиви є швидкими. Проте зі зростанням потужності вебзастосунків, додаванням можливостей штибу роботи з аудіо та відео, звертання до необроблених даних при роботі з [вебсокетами](/uk/docs/Web/API/WebSockets_API) й так далі стало очевидним, що трапляються випадки, коли було б корисним, якби код JavaScript міг швидко й легко працювати з необробленими двійковими даними в типізованих масивах.

Типізовані масиви поділяють чимало тих самих методів, що й масиви, з подібною семантикою. Проте типізовані масиви концептуально масивами не є і не мають всіх методів масивів.

### Буфери та представлення: архітектура типізованого масиву

Для досягнення максимальних гнучкості й ефективності типізовані масиви JavaScript ділять свою реалізацію на _буфери_ та _представлення_. Буфер (реалізований в об'єкті {{jsxref("ArrayBuffer")}}) – це об'єкт, що представляє порцію даних; він не має виразного формату і не надає механізму для звертання до його вмісту. Для звертання до пам'яті, вміщеної в буфері, треба використати представлення. Представлення надає _контекст_: тип даних, початковий відступ та кількість елементів, – що перетворює дані на фактичний типізований масив.

![Діаграма, що демонструє, як різні типізовані масиви можуть бути представленнями для єдиного буфера. Усі вони мають різну кількість елементів та ширину.](typed_arrays.png)

### ArrayBuffer

{{jsxref("ArrayBuffer")}} – тип даних, що використовується для представлення узагальненого двійкового буфера даних з фіксованою довжиною. Не можна безпосередньо працювати зі вмістом `ArrayBuffer`; натомість створюється представлення типізованого масиву чи {{jsxref("DataView")}}, котре представляє буфер у конкретному форматі й використовує такий формат для зчитування та запису вмісту буфера.

### Представлення типізованих масивів

Представлення типізованих масивів мають промовисті назви й пропонують представлення для всіх звичних числових типів, як то `Int8`, `Uint32`, `Float64` і так далі. Існує одне особливе представлення типізованого масиву, {{jsxref("Uint8ClampedArray")}}, котре обмежує значення діапазоном `0` і `255`. Це корисно для [Обробки даних полотна](/uk/docs/Web/API/ImageData), наприклад.

Більше інформації – на сторінці [Типізованих масивів JavaScript](/uk/docs/Web/JavaScript/Typed_arrays) та в довідковій документації різних об'єктів {{jsxref("TypedArray")}}.
