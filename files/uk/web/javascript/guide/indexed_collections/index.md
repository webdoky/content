...
> [!NOTE]
> У коді вище `arrayLength` мусить належати до типу `Number`. Інакше – буде створено масив з єдиним елементом (переданим значенням). Виклик `arr.length` поверне `arrayLength`, але масив не містить жодних елементів. Цикл {{jsxref("Statements/for...in", "for...in")}} не знайде в масиві жодних властивостей.
...
> [!NOTE]
> Також [засоби доступу до властивостей](/uk/docs/Web/JavaScript/Reference/Operators/Property_accessors) можна використовувати для звертання до інших властивостей масиву, як з об'єктом.
>
> ```js
...
> [!NOTE]
> Якщо, в наведеному вище коді, передати оператору масиву значення, що не є цілим числом, то в об'єкті, що представляє масив, буде створена властивість, а не елемент масиву.
>
> ```js
> const arr = [];
> arr[3.4] = "Помаранчі";
...
Поширена операція – ітерування значень масиву, обробка кожного з них у певний спосіб, отак:
...
> [!NOTE]
> Метод `sort()`, описаний вище, не є ітеративним методом, тому що його функція зворотного виклику використовується лише для порівняння і може не бути викликана у певному відомому на основі послідовності елементів порядку. Також `sort()` не приймає параметр `thisArg`.
...
Тут – інвентарний масив, що містить об'єкти "їжі", котрі мають `name` (назву) та `type` (тип).
