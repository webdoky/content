...
- [Інструкція з підписом](#instruktsiia-z-pidpysom)
...
У прикладі нижче функція містить інструкцію `for`, котра підраховує кількість обраних у прокрутному списку варіантів (елементі [`<select>`](/uk/docs/Web/HTML/Element/select), що дозволяє вибрати декілька варіантів).
...
## Інструкція з підписом

{{jsxref("statements/label","Підпис")}} дає інструкції ідентифікатор, що дає змогу звертатися до неї деінде в програмі. Наприклад, можна використати підпис для ідентифікації циклу, а потім використати інструкції `break` чи `continue`, аби вказати, коли програма повинна перервати цикл чи продовжити його виконання.

Синтаксис інструкції з підписом має наступний вигляд:
...
Значення `label` може бути будь-яким ідентифікатором JavaScript, що не є зарезервованим словом. `statement`, котра ідентифікується підписом, може бути будь-якою інструкцією.

### Приклад

В цьому прикладі підпис `markLoop` ідентифікує цикл `while`.
...
Інструкцію {{jsxref("statements/break","break")}} слід використовувати для завершення циклу чи `switch`, або вкупі з інструкцією з підписом.

- Коли `break` використовується без підпису, вона негайно завершує `while`, `do-while`, `for` або
  `switch` найглибшого рівня вкладеності, що її оточує, і передає контроль виконання до наступної інструкції.
- Коли `break` використовується з підписом, вона завершує вказану інструкцію з підписом.
...
1. Перша форма синтаксису завершує цикл чи `switch` навколо, що має найглибшу вкладеність.
...
### Приклад 2: переривання за підписом
